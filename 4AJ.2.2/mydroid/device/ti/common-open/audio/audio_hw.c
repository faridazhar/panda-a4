...
static void set_eq_filter(struct omap_audio_device *adev)
{
    ...
    if (!adev->dl2_support)
        dl1_eq_applicable |= adev->devices & AUDIO_DEVICE_OUT_SPEAKER;
    ...
    else if (!adev->dl2_support && (adev->devices & AUDIO_DEVICE_OUT_SPEAKER))
    ...
    if (out) {
        ...
        for(i = 0; i < MAX_OUTPUTS; i++) {
            if (adev->selected_device[i].id)
                adev->devices |= adev->selected_device[i].device;
        }
    }
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        ...
        pthread_mutex_lock(&adev->lock);
        pthread_mutex_lock(&out->lock);
        if (((out->device & AUDIO_DEVICE_OUT_ALL) != val) && (val != 0)) {
            for (i = 0; i< MAX_OUTPUTS; i++) {
                if (out == adev->active_output[i]) {
                    do_output_standby(out);
