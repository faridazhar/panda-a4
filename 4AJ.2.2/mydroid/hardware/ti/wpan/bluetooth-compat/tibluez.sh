...
function main() {
	...
	if [ "$?" -ne 0 ]; then
        ...
        if [ "$1" ==  "$CREATE_PATCH_PARAM" ]; then
		    ...
		    #create new patches
		    diff -x 'Makefile' -x 'Makefile.bk' -ruN "$ADMIN_UP_RES_DIR/net" net > "$TI_COMPAT_PATCH";
		    diff -x 'compat_autoconf.h' -ruN "$ADMIN_UP_RES_DIR/include" include >> "$TI_COMPAT_PATCH";
		    diff  -x 'Makefile' -x 'Makefile.bk' -ruN "$ADMIN_UP_RES_DIR/drivers" drivers >> "$TI_COMPAT_PATCH";
