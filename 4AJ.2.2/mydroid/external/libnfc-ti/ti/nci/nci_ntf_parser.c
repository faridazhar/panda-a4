...
nfc_u32 nci_parse_ntf(nfc_u16 opcode, nfc_u8 *p_data, nci_ntf_e *p_ntf_id, nci_ntf_param_u *pu_ntf, nfc_u32 len)
{
	...
	switch(opcode)
	{
		...
		break;
		case NCI_OPCODE_CORE_RF_FIELD_INFO_NTF:
		{
			...
			*p_ntf_id = E_NCI_NTF_CORE_RF_FIELD_INFO;
		}
		break;
		...
		break;
		case NCI_OPCODE_RF_ACTIVATE_NTF:
		{
			nfc_u8 *p_data_base = p_data;
			NCI_GET_1B(p_data, pu_ntf->activate.target_handle);
			NCI_GET_1B(p_data, pu_ntf->activate.selected_rf_protocol);
			NCI_GET_1B(p_data, pu_ntf->activate.detected_rf_technology_and_mode);
			NCI_GET_1B(p_data, pu_ntf->activate.length_rf_technology_specific_params);
			switch(pu_ntf->activate.detected_rf_technology_and_mode)
			{
				case NFC_A_PASSIVE_POLL_MODE:
				{
					struct nci_activate_nfca_poll *poll_a = &pu_ntf->activate.rf_technology_specific_params.nfca_poll;
				...
				{
					struct nci_activate_nfcb_poll *poll_b = &pu_ntf->activate.rf_technology_specific_params.nfcb_poll;
				...
				{
					struct nci_activate_nfcf_poll *poll_f = &pu_ntf->activate.rf_technology_specific_params.nfcf_poll;
					...
					if(pu_ntf->activate.rf_interface_type == NCI_RF_INTERFACE_NFC_DEP)
					{
						if(pu_ntf->activate.activation_params_len > 0)
						{
							...
							NCI_GET_1B(p_data, nfc_dep->atr_res_length);
							//TBD probably FW bug - workaround - remove
							nfc_dep->atr_res_length -=1;
							p_data++;
							//////////////////////////////////////////
							NCI_GET_ARR(p_data, nfc_dep->atr_res, nfc_dep->atr_res_length);
						}
				...
				{
					struct nci_activate_nfcb_listen *listen_b = &pu_ntf->activate.rf_technology_specific_params.nfcb_listen;
				...
				{
					struct nci_activate_nfcf_listen *listen_f = &pu_ntf->activate.rf_technology_specific_params.nfcf_listen;

					if(pu_ntf->activate.length_rf_technology_specific_params > 0)
					{
						//TBD - return when fixed bug in FW
						/*NCI_GET_1B(p_data, listen_f->local_nfcid2_len);
						NCI_GET_ARR(p_data, listen_f->local_nfcid2, listen_f->local_nfcid2_len);*/

						//TBD - remove when fixed bug in FW
						listen_f->local_nfcid2_len = pu_ntf->activate.length_rf_technology_specific_params;
						NCI_GET_ARR(p_data, listen_f->local_nfcid2, pu_ntf->activate.length_rf_technology_specific_params);
					}
			...
			*p_ntf_id = E_NCI_NTF_RF_ACTIVATE;
		}
		break;
		...
		{
			NCI_GET_1B(p_data, pu_ntf->nfcee_discover.target_handle);
		...
		{
			NCI_GET_1B(p_data, pu_ntf->generic_error.status);
			osa_report(ERROR, ("Received ERROR NCI_OPCODE_CORE_GENERIC_ERROR_NTF with status %d",pu_ntf->generic_error.status));
		...
		case NCI_OPCODE_CE_ACTION_NTF:
		case NCI_OPCODE_TEST_RF_CONTROL_NTF:
		{
			/* Not Supported */
			osa_report(ERROR, ("Received NOT SUPPORTED NTF!!!!"));
			OSA_ASSERT(0);
		}
		break;
	}
