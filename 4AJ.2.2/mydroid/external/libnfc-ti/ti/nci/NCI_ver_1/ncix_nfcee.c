...
#define NFCEE_INVALID_NFCEE_ID (0xff)
...
*		nfcee_id -Target handle received from NAL (SE \ UICC)
...
*
*******************************************************************************/
static void static_send_nfcee_mode_set(struct ncix_nfcee *p_nfcee, nfc_u8 nfcee_id, nfc_u8 mode, rsp_cb_f f_rsp)
{
...
/* Send NCI_OPCODE_NFCEE_MODE_SET_CMD */
	...
	nci_cmd.nfcee_mode_set.nfcee_id = nfcee_id;
...
static void static_mode_set_rsp(nfc_handle_t h_nfcee, nfc_u16 opcode, nci_rsp_param_u *pu_rsp)
{
	...
	{
		nci_sm_process_evt(p_nfcee->h_sm, E_EVT_NFCEE_MODE_SET_RSP_OK);
	}
	else
	{
		nci_sm_process_evt(p_nfcee->h_sm, E_EVT_NFCEE_MODE_SET_RSP_FAIL);
	}
}
...
*
*******************************************************************************/
static void static_send_nfcee_conn_create(struct ncix_nfcee *p_nfcee, nfc_u8 nfcee_id, rsp_cb_f f_rsp)
{
	nci_cmd_param_u nci_cmd;
	nci_status_e status = NCI_STATUS_OK;
	struct nci_tlv tlv;
	...
	nci_cmd.conn_create.destination_type = NCI_DESTINATION_TYPE_NFCEE;
	val[1] = nfcee_id;
	val[0] = p_nfcee->nfcee_protocols[0];
	...
	nci_cmd.conn_create.num_of_tlvs = 1;
	nci_cmd.conn_create.p_tlv = &tlv;
...
static void static_conn_create_rsp(nfc_handle_t h_nfcee, nfc_u16 opcode, nci_rsp_param_u *pu_rsp)
{
	...
	if(NCI_STATUS_OK == pu_rsp->generic.status)
	{
		...
		ncix_get_rsp_ptr(p_nfcee->h_ncix)->nfcee.nfcee_id = p_nfcee->nfcee_id;
		...
		nci_sm_process_evt(p_nfcee->h_sm, E_EVT_CORE_CONN_CREATE_RSP_OK);
	}
	else
	{
		nci_sm_process_evt(p_nfcee->h_sm, E_EVT_CORE_CONN_CREATE_RSP_FAIL);
	}
}
...
static void static_conn_close_rsp(nfc_handle_t h_nfcee, nfc_u16 opcode, nci_rsp_param_u *pu_rsp)
{
	...
	if(NCI_STATUS_OK == pu_rsp->generic.status)
	{
		nci_sm_process_evt(p_nfcee->h_sm, E_EVT_CORE_CONN_CLOSE_RSP_OK);
	}
	else
	{
		nci_sm_process_evt(p_nfcee->h_sm, E_EVT_CORE_CONN_CLOSE_RSP_FAIL);
	}
}
...
nfc_u32 action_evt_switch_to_rf_comm(nfc_handle_t h_nfcee, nfc_u32 curr_state)
{
	...
	{
	case E_STATE_OFF:
		static_send_nfcee_mode_set(p_nfcee, p_nfcee->nfcee_id, NCI_NFCEE_MODE_ACTIVATE, static_mode_set_rsp);
...
nfc_u32 action_evt_switch_to_host_comm(nfc_handle_t h_nfcee, nfc_u32 curr_state)
{
	...
	{
	case E_STATE_OFF:
		static_send_nfcee_mode_set(h_nfcee, p_nfcee->nfcee_id, NCI_NFCEE_MODE_ACTIVATE, static_mode_set_rsp);
		next_state = E_STATE_ACTIVATING_HOST_COMM;
		break;
	case E_STATE_RF_COMM:
		//Send CORE_CONN_CREATE_CMD(tgt_hndl)
		static_send_nfcee_conn_create(p_nfcee, p_nfcee->nfcee_id, static_conn_create_rsp);
...
nfc_u32 action_evt_switch_to_off(nfc_handle_t h_nfcee, nfc_u32 curr_state)
{
	...
	case E_STATE_OFF:
		...
		break;
	case E_STATE_RF_COMM:
		static_send_nfcee_mode_set(p_nfcee, p_nfcee->nfcee_id, NCI_NFCEE_MODE_DEACTIVATE, static_mode_set_rsp);
...
nfc_u32 action_evt_mode_set_rsp_ok(nfc_handle_t h_nfcee, nfc_u32 curr_state)
{
	...
	case E_STATE_DE_ACTIVATING:
		//Send operation complete
		ncix_op_complete(p_nfcee->h_ncix, NCI_STATUS_OK);
		nfcee_remove_discovery_request(p_nfcee->h_nci, p_nfcee->nfcee_id);
		next_state = E_STATE_OFF;
		break;
	default:
		OSA_ASSERT(0);
...
static struct nci_sm_state s_nfcee_state[] =
{
...
static struct nci_sm_event s_nfcee_event[] =
{
...
*
* Description: find free entry (invalid nfcee_id) in NFCEE interfaces database
*
...
static struct ncix_nfcee* find_free_nfcee(struct ncix_nfcees_db *p_db)
{
	...
	{
		if(NULL == p_db->p_nfcees[i].h_sm && NFCEE_INVALID_NFCEE_ID == p_db->p_nfcees[i].nfcee_id)
		{
...
* Name: find_nfcee
*
* Description: find entry in NFCEE interfaces database that its nfcee_id is same as incoming param nfcee_id
*
* Input: 	p_db - Handle to NFCEE interfaces database
*		nfcee_id - target handle to find
*
* Output: None
*
...
*
*******************************************************************************/
static struct ncix_nfcee* find_nfcee(struct ncix_nfcees_db *p_db, nfc_u8 nfcee_id)
{
	nfc_u8 i = 0;
	for(i = 0; i < p_db->num_nfcees; i++)
	{
		if(nfcee_id == p_db->p_nfcees[i].nfcee_id)
		{
			return &(p_db->p_nfcees[i]);
		}
	}
	...
	case NFCEE_STATUS_CONNECTED_AND_ACTIVE:
		{
			...
			if(NULL != p_nfcee)
			{
				p_nfcee->h_sm = nci_sm_create(p_nfcee, "NFCEE", E_STATE_OFF, s_nfcee_state, s_nfcee_event);

				//TBD - put back after fix in FW instead the line above
				//p_nfcee->h_sm = nci_sm_create(p_nfcee, "NFCEE", E_STATE_RF_COMM, s_nfcee_state, s_nfcee_event);
			}
			else
			{
				OSA_ASSERT(0);
			...
			p_nfcee->nfcee_id = pu_ntf->nfcee_discover.nfcee_id;

			//TBD - remove after fix in FW
			//Deactivate the NFCEE - it will be activated per user request
			static_send_nfcee_mode_set(p_nfcee, p_nfcee->nfcee_id, NCI_NFCEE_MODE_DEACTIVATE, NULL);
	...
	case NFCEE_STATUS_REMOVED:
		{
			...
			//Signal the NAL -??? //TBD
			p_nfcee = find_nfcee(p_db, pu_ntf->nfcee_discover.nfcee_id);
			if(NULL != p_nfcee)
			{
				nci_sm_destroy(p_nfcee->h_sm);
				osa_mem_zero(p_nfcee, sizeof(struct ncix_nfcee));
				p_nfcee->nfcee_id = NFCEE_INVALID_NFCEE_ID;
...
static void hndlr_op_nfcee(nfc_handle_t h_nfcee_db, op_param_u *pu_param)
{
	...
	p_nfcee = find_nfcee(p_db, p_op_param->nfcee_id);
	if(NULL == p_nfcee)
	{
	...
	ncix_get_rsp_ptr(p_nfcee->h_ncix)->nfcee.nfcee_id = p_op_param->nfcee_id;
	...
	case E_SWITCH_RF_COMM:
		{
			nci_sm_process_evt(p_nfcee->h_sm, E_EVT_SWITCH_TO_RF_COMM_C);
			break;
		}
	case E_SWITCH_HOST_COMM:
		{
			nci_sm_process_evt(p_nfcee->h_sm, E_EVT_SWITCH_TO_HOST_COMM_C);
			break;
		}
	case E_SWITCH_OFF:
		{
			nci_sm_process_evt(p_nfcee->h_sm, E_EVT_SWITCH_TO_OFF_C);
			break;
		}
	default:
		{
...
nfc_handle_t ncix_nfcee_create(nfc_handle_t h_ncix, nfc_handle_t h_nci)
{
	...
	for(i = 0; i < p_db->num_nfcees; i++)
	{
		p_db->p_nfcees[i].nfcee_id = NFCEE_INVALID_NFCEE_ID;
...
void ncix_nfcee_destroy(nfc_handle_t h_nfcee_db)
{
	...
	for(i = 0; i < p_db->num_nfcees; i++)
	{
		...
		{
			nci_sm_destroy(p_db->p_nfcees[i].h_sm);
		}
	}
