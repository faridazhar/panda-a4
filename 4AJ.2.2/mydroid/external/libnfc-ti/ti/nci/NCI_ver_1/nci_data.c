...
struct nci_conn
{
	...
	nfc_handle_t		h_tx_timer;
	nfc_u8				conn_id;
	struct nci_data_context* p_owner;
};
...
void tx_timer_cb(struct nci_conn *p_conn)
{
	osa_report(INFORMATION, ("NCI data - tx_timer_cb!"));
	p_conn->p_owner->f_rx_ind_cb(p_conn->p_owner->h_data_cb, p_conn->conn_id, NULL, 0, NCI_STATUS_RF_TIMEOUT_ERROR);
...
static void static_activate_ntf(nfc_handle_t h_data, nfc_u16 opcode, nci_ntf_param_u *pu_ntf)
{
	...
	nci_data_activate_rf_conn(h_data, pu_ntf->activate.initial_num_of_credits_rf_conn, pu_ntf->activate.max_data_packet_payload_size);
}
...
static void static_core_intf_err__ntf(nfc_handle_t h_data, nfc_u16 opcode, nci_ntf_param_u *pu_ntf)
{
	struct nci_data_context *p_ctx = (struct nci_data_context *)h_data;
	OSA_UNUSED_PARAMETER(opcode);
	OSA_UNUSED_PARAMETER(pu_ntf);

	switch(pu_ntf->core_iface_error.status)
	{
	case NCI_STATUS_RF_TRANSMISSION_ERROR:
	case NCI_STATUS_RF_PROTOCOL_ERROR:
	case NCI_STATUS_RF_TIMEOUT_ERROR:
		{
			p_ctx->f_rx_ind_cb(p_ctx->h_data_cb, pu_ntf->core_iface_error.connID, NULL, 0, pu_ntf->core_iface_error.status);
			break;
		}
	}
}
...
*
*******************************************************************************/
nci_status_e nci_data_prepare_2_send(nfc_handle_t h_data, nfc_u8 conn_id, nfc_u8 *p_user_payload, nfc_u32 user_payload_len, nfc_u32 tx_timeout)
{
	struct nci_data_context *p_ctx = (struct nci_data_context *)h_data;
	nfc_u8 *p_data;
	...
	while(total_len)
	{
		...
		p_nci_data->tx_timeout = tx_timeout;
...
void nci_data_send(nfc_handle_t h_data, nfc_handle_t h_nci_data)
{
	...
	if(p_conn->b_active_conn && (p_conn->instance == p_nci_data->conn_instance))
	{
		...
		if(0 != p_nci_data->tx_timeout)
		{
			if(0 != p_conn->h_tx_timer)
			{
				osa_timer_stop(p_conn->h_tx_timer);
			}
			p_conn->h_tx_timer = osa_timer_start(p_ctx->h_osa, p_nci_data->tx_timeout, tx_timer_cb, p_conn);
		}
	}
	else
	{
...
*
*******************************************************************************/
void nci_data_activate_rf_conn(nfc_handle_t h_data, nfc_u8 initial_num_of_credits, nfc_u8 max_data_packet_payload_size)
{
	struct nci_data_context *p_ctx = (struct nci_data_context *)h_data;
	...
	p_conn->maximum_packet_payload_size = max_data_packet_payload_size;
	p_conn->initial_fc_credit = initial_num_of_credits;
	p_conn->current_fc_credit = p_conn->initial_fc_credit;
...
void nci_data_open(nfc_handle_t h_nci, nfc_handle_t h_data)
{
	struct nci_data_context *p_ctx = (struct nci_data_context*)h_data;


	/* register to relevant notifications */
	nci_register_ntf_cb(h_nci, E_NCI_NTF_CORE_CONN_CREDITS, static_conn_credit_ntf, p_ctx);
	nci_register_ntf_cb(h_nci, E_NCI_NTF_RF_INTF_ACTIVATED, static_activate_ntf, p_ctx);
	nci_register_ntf_cb(h_nci, E_NCI_NTF_RF_DEACTIVATE, static_deactivate_ntf, p_ctx);
	nci_register_ntf_cb(h_nci, E_NCI_NTF_CORE_INTERFACE_ERROR, static_core_intf_err__ntf, p_ctx);
}

void nci_data_close(nfc_handle_t h_nci, nfc_handle_t h_data)
{
	struct nci_data_context *p_ctx = (struct nci_data_context *)h_data;
	nfc_u8 i = 0;
	...
	for(i=0; i<MAX_LOGICAL_CONNS; i++)
	{
		if(NULL != p_ctx->p_conn_tbl[i].h_tx_timer)
		{
			osa_timer_stop(p_ctx->p_conn_tbl[i].h_tx_timer);
			p_ctx->p_conn_tbl[i].h_tx_timer = NULL;
		}
	}
	...
	nci_unregister_ntf_cb(h_nci, E_NCI_NTF_CORE_CONN_CREDITS, static_conn_credit_ntf);
	nci_unregister_ntf_cb(h_nci, E_NCI_NTF_RF_INTF_ACTIVATED, static_activate_ntf);
	nci_unregister_ntf_cb(h_nci, E_NCI_NTF_RF_DEACTIVATE, static_deactivate_ntf);
	nci_unregister_ntf_cb(h_nci, E_NCI_NTF_CORE_INTERFACE_ERROR, static_core_intf_err__ntf);
}
