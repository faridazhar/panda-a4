...
nci_status_e nci_generate_cmd(nfc_u16 opcode, nci_cmd_param_u *pu_cmd, struct nci_buff **pp_nci_buff, nfc_handle_t h_nci)
{
	...
	switch(opcode)
	{
		...
		case NCI_OPCODE_NFCEE_MODE_SET_CMD:
			...
			p_data = nci_buff_data(p_buff) + NCI_CTRL_HEADER_SIZE;
			NCI_SET_1B(p_data, pu_cmd->nfcee_mode_set.target_handle);
		...
		case NCI_OPCODE_CORE_CONN_CREATE_CMD:
			{
				...
				p_buff = nci_buff_alloc(NCI_CTRL_HEADER_SIZE + NCI_CORE_CONN_CREATE_CMD_MIN_SIZE + pu_cmd->conn_create.total_length);
				p_data = nci_buff_data(p_buff) + NCI_CTRL_HEADER_SIZE;
				p_data_base = p_data;

				NCI_SET_1B(p_data, pu_cmd->conn_create.target_handle);
