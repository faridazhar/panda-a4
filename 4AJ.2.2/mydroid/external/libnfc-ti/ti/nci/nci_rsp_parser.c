...
nfc_u32 nci_parse_rsp(nfc_u16 opcode, nfc_u8 *p_data, nci_rsp_param_u *pu_rsp, nfc_u32 len)
{
	...
	switch(opcode)
	{
		...
		{
			NCI_GET_ARR(p_data, pu_rsp->init.nci_features.array, NCI_CORE_INIT_RSP_FEATURES_SIZE);
			...
			NCI_GET_2B(p_data, pu_rsp->init.rf_sending_buffer_size);
			NCI_GET_2B(p_data, pu_rsp->init.rf_receiving_buffer_size);
			NCI_GET_ARR(p_data, pu_rsp->init.manufacturer_id.array, NCI_CORE_INIT_RSP_MANUFACTURER_ID_SIZE);
