...
static
void
phFriNfc_Tpz_H_RemoveAddrByte(uint16_t *len, uint8_t *buf)
{
	if ((*len) > 0) {
		(*len)--;
		if ((*len) > 0)
			(void)memcpy ((void *)buf, (void *)(buf + 1), (*len));
	}
}
#endif
    ...
    phFriNfc_NdefMap_t          *psNdefMap)
{
    ...
    switch (e_readonly_seq)
    {
        ...
        {
#ifdef HAL_NFC_TI_DEVICE
	phFriNfc_Tpz_H_RemoveAddrByte(psNdefMap->SendRecvLength, psNdefMap->SendRecvBuf);
#endif
