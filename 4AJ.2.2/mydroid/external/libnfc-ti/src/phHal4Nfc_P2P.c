...
#endif
			PHDBG_INFO("HAL4:Calling nci_send_data()");

#if (NCI_VERSION_IN_USE==NCI_DRAFT_9)
			status = nci_send_data(Hal4Ctxt->nciHandle, 0, sTransferData.buffer, sTransferData.length);
#elif (NCI_VERSION_IN_USE==NCI_VER_1)
			status = nci_send_data(Hal4Ctxt->nciHandle, 0, sTransferData.buffer, sTransferData.length, 0);
#endif
				 ...
				 )
{
	...
	hal4nfc_report((" phHal4Nfc_Receive"));
...
static void phHal4Nfc_P2PRecvTimerCb(uint32_t P2PRecvTimerId, void *pContext)
{
	...
	hal4nfc_report((" phHal4Nfc_P2PRecvTimerCb"));
...
void phHal4Nfc_SendCompleteHandler(phHal4Nfc_Hal4Ctxt_t  *Hal4Ctxt,void *pInfo)
{
	...
	hal4nfc_report((" phHal4Nfc_SendCompleteHandler"));
...
void phHal4Nfc_RecvCompleteHandler(phHal4Nfc_Hal4Ctxt_t  *Hal4Ctxt,void *pInfo)
{
	...
	hal4nfc_report((" phHal4Nfc_RecvCompleteHandler"));
							...
							)
{
	...
	hal4nfc_report((" phHal4Nfc_P2PActivateComplete"));
							   ...
							   )
{
	...
	hal4nfc_report((" phHal4Nfc_HandleP2PDeActivate"));
