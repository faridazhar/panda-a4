...
/* Amir: Bug fix - Transport was never cleaned in case of link deactivate */
NFCSTATUS phFriNfc_LlcpTransport_Clear (phFriNfc_LlcpTransport_t *pLlcpTransport)
{
   NFCSTATUS status = NFCSTATUS_SUCCESS;
   ...
   {
      LLCP_PRINT("Amir: phFriNfc_LlcpTransport_Clear");

      /* Clear Transport structure */
      pLlcpTransport->bSendPending     = FALSE;
      pLlcpTransport->bRecvPending     = FALSE;
      pLlcpTransport->bDmPending       = FALSE;
      pLlcpTransport->bFrmrPending     = FALSE;
      pLlcpTransport->socketIndex      = 0;
      pLlcpTransport->LinkStatusError  = 0;
   }
   return status;
}
