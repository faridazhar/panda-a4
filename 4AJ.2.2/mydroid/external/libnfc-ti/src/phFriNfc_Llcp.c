...
static void phFriNfc_Llcp_ResetLTO( phFriNfc_Llcp_t *Llcp )
{
   ...
   if (Llcp->state == PHFRINFC_LLCP_STATE_OPERATION_RECV)
   {
      ...
      /* Bug fix: Timeout is not enough when using large LLCP packets.*/
      nDuration = Llcp->sRemoteParams.lto * 10 * 2;
#else
...
#ifdef HAL_NFC_TI_DEVICE
      /* Bug fix: Timeout is not enough when using large LLCP packets.*/
      nDuration = (Llcp->sLocalParams.lto * 10);
#else
...
#ifdef HAL_NFC_TI_DEVICE
   /* Bug fix: Race condition - SYMM packet was sent right after P2P disconnection.
               Don't send anything if in or after deactivation phase. */
