...
#define report(msg)                     osa_report(INFORMATION, msg)
...
NFCSTATUS phLibNfc_Mgt_ConfigureDriver (pphLibNfc_sConfig_t     psConfig,
										void **                 ppDriverHandle)
{
	report((" phLibNfc_Mgt_ConfigureDriver"));
...
{
	report((" phLibNfc_Mgt_UnConfigureDriver"));
								 ...
								 void                 *pContext)
{
	...
	report((" phLibNfc_Mgt_Initialize"));
...
{
	report((" phLibNfc_DefaultHandler"));
								   ...
								   )
{
	...
	report((" phLibNfc_Mgt_DeInitialize"));
...
#ifdef HAL_NFC_TI_DEVICE
			/* store mode to be used later in phLibNfc_Pending_Shutdown*/
			pLibContext->close_mode = mode;
#endif
...
void phLibNfc_Pending_Shutdown(void)
{
	...
	report((" phLibNfc_Pending_Shutdown"));
...
#ifdef HAL_NFC_TI_DEVICE
						(void *)gpphLibContext,
						gpphLibContext->close_mode);
	gpphLibContext->close_mode = 0;
#else
						(void *)gpphLibContext);
#endif
					...
					void                         *pContext)
{
	...
	report((" phLibNfc_Mgt_GetstackCapabilities"));
								 ...
								 void                *pContext)
{
	...
	report((" phLibNfc_Mgt_ConfigureTestMode"));
