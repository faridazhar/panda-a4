...
#define DBG(fmt, arg...)  printf("MEDIA: %s: " fmt "\n" , __FUNCTION__ , ## arg)
...
struct media_player {
	...
	uint8_t			status;
	uint32_t		position;
	uint8_t			abs_volume_val;
	uint8_t			volume;
        char                    absolute_volume[4];
	GCompatTimer		*timer;
};
...
static const char *attrval_to_str(uint8_t attr, uint8_t value)
{
	...
	case AVRCP_ATTRIBUTE_SCAN:
		...
		break;
                /* AVRCP1.4 version support- yes/no */
        case AVRCP_14_VERSION:
                switch (value) {
                    case AVRCP_1_4_VER_ON:
                        return "yes";
                    case AVRCP_1_4_VER_OFF:
                        return "no";
                }
                break;
        }
...
static int set_setting(uint8_t attr, uint8_t val, void *user_data)
{
	...
	property = attr_to_str(attr);
        if(attr == AVRCP_14_VOLUME) {
                if(val == mp->abs_volume_val) {
                        temp_val = mp->volume;
                } else {
                        /* Convert absolute volume (0-127) to index volume (0-15) */
                        temp_val = (uint16_t) val;
                        temp_val *= 15;
                        temp_val /= 127;
                }
                // Get the volume into string
                sprintf(num_str, "%02d", (int) temp_val); // Converts to decimal base.
                DBG("volume string=%s", num_str);
                // Copy first 2 chars 0-15
                mp->absolute_volume[0] = num_str[0];
                mp->absolute_volume[1] = num_str[1];
                // Terminate string
                mp->absolute_volume[2] = 0;
                value = mp->absolute_volume;
                // Save current media player volume index
                mp->volume = (uint8_t) temp_val;
        } else {
	...
	if (property == NULL || value == NULL)
        ...
        if(attr < AVRCP_14_VOLUME) {
            if (!g_hash_table_lookup(mp->settings, GUINT_TO_POINTER(attr_uint))) {
                DBG("!g_hash_table_lookup() FAILED!!!");
                return -EINVAL;
            }
        }
...
static uint64_t get_uid(void *user_data)
{
	...
	pid = get_metadata(AVRCP_MEDIA_ATTRIBUTE_TRACK, user_data);
	DBG("pid=%p", pid);

	return (uint64_t)GPOINTER_TO_UINT(pid);
}
...
static uint32_t get_position(void *user_data)
{
	...
	timedelta = g_compat_timer_elapsed(mp->timer, NULL);
...
/* Blueti: Just change the media player volume */
static void set_volume(uint8_t volume, struct audio_device *dev, void *user_data)
{
	struct media_player *mp = user_data;

	if (mp->abs_volume_val == volume)
		return;
	else
		DBG("callback volume value different from media player one- mp->abs_volume = 0x%x volume from callback = 0x%x", mp->abs_volume_val, volume);
...
static gboolean set_absolute_volume(struct media_player *mp, DBusMessageIter *iter)
{
        const char *value;
        uint16_t temp_val;
        uint8_t absolute_volume_val;
        uint8_t volume_index_val;
        struct metadata_value *duration;
        ...
        dbus_message_iter_get_basic(iter, &value);
        DBG("Volume string=%s", value);
        temp_val = (uint16_t) atoi(value);
        volume_index_val = (uint8_t) temp_val;
        temp_val *= 127;
        temp_val /= 15;
        absolute_volume_val = (uint8_t) temp_val;
        DBG("absolute_volume_val=%u", absolute_volume_val);
        DBG("Last volume index=%u", mp->volume);

        if(mp->volume != volume_index_val) {
                mp->volume = volume_index_val;
                mp->abs_volume_val = absolute_volume_val;
                g_timer_start(mp->timer);
                DBG("Send avrcp_set_volume() with volume=%u", absolute_volume_val);
                avrcp_set_player_volume(mp->player, absolute_volume_val);
        }
							...
							DBusMessageIter *entry)
{
	...
	if (strcasecmp(key, "Position") == 0)
        ...
        if (strcasecmp(key, "Volume") == 0){
            return set_absolute_volume(mp, &var);
        }
...
/* Android does not use these methods and due to using D-Bus < 1.3, they
 * cannot be enabled
 */
#ifndef ANDROID
