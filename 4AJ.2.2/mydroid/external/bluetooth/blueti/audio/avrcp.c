...
#define DBG(fmt, arg...)  printf("AVRCP: %s: " fmt "\n" , __FUNCTION__ , ## arg)
...
// AVRCP 1.4
static gboolean avrcp_handle_volume_notification(struct avctp *session,
						...
						uint8_t transaction)
{
	...
	memcpy(&identifier, &pdu->params[0],sizeof(uint64_t));
					...
					void *user_data)
{
	...
	volume = pdu->params[1] & 0x7F;
        DBG(" pdu->company_id[0]=%u, pdu->company_id[1]=%u, pdu->company_id[2]=%u",
            pdu->company_id[0], pdu->company_id[1], pdu->company_id[2]);
        DBG(" pdu->pdu_id=%u", pdu->pdu_id);
        DBG(" pdu->packet_type=%u", pdu->packet_type);
        DBG(" pdu->params_len=0x%x", pdu->params_len);
        DBG(" volume value, pdu->params[1]=%u", pdu->params[1]);

	if (code == AVC_CTYPE_CHANGED) {
            DBG("Send over DBUS- volume=%u", volume);
            if(player_set_attribute(player, AVRCP_14_VOLUME, volume) < 0)
                DBG("player_set_attribute FAILED!!!\n");
            DBG(" Register again for volume changed");
		register_volume_notification(player);
		return FALSE; // In order to delete the last callback handler
	}
	return TRUE;
}
...
static void register_volume_notification(struct avrcp_player *player)
{
    ...
    DBG(" player=%p, player->dev=%p", player, player->dev);
	avctp_send_vendordep_req(player->session, AVC_CTYPE_NOTIFY,
					AVC_SUBUNIT_PANEL, buf, length,
                                        avrcp_handle_volume_notification, player);
}
				...
				avctp_state_t new_state, void *user_data)
{
	...
	case AVCTP_STATE_CONNECTING:
		...
		player->dev = dev;
        DBG(" Got AVCTP_STATE_CONNECTING, player->handler=0x%x, player->session=%p, player->dev=%p",
                player->handler, player->session, player->dev);
		if (!player->handler)
	...
	case AVCTP_STATE_CONNECTED:
		...
		desc = list->data;
                ...
                DBG("AVCTP_STATE_CONNECTED version=0x%04X",desc->version);
                if (desc && desc->version >= 0x0104) {
			register_volume_notification(player);
                        if(player_set_attribute(player, AVRCP_14_VERSION, AVRCP_1_4_VER_ON) < 0)
                            DBG("player_set_attribute FAILED!!!\n");
                }
					...
					void *user_data)
{
	...
	player->cb->set_volume(volume, player->dev, player->user_data);
        DBG("Set attribute to send over DBUS the volume=%d", volume);
        if(player_set_attribute(player, AVRCP_14_VOLUME, volume) < 0)
            DBG("player_set_attribute FAILED!!!\n");
...
int avrcp_set_player_volume(void *av_player, uint8_t volume)
{
    struct avrcp_server *server;
    struct avrcp_player *player = av_player;
    uint8_t buf[AVRCP_HEADER_LENGTH + 1];
    struct avrcp_header *pdu = (void *) buf;
    struct audio_device *dev = player->dev;
