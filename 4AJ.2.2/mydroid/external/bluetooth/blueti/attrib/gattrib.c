...
static void notify_pending_commands_of_timeout(struct _GAttrib *attrib)
{
	struct command *c;

	c = g_queue_pop_head(attrib->queue);
	if (c) {
		if (c->func)
	...
	/*
	 * Since the GATT is dropped after the timeout, we notify the remaining
	 * pending commands that they were dropped
	 */
	while ((c = g_queue_pop_head(attrib->queue))) {
		if (c->func) {
			c->func(ATT_ECODE_COMMAND_ABORTED, NULL, 0, c->user_data);
			command_destroy(c);
		}
	}
	while ((c = g_queue_pop_head(attrib->response_queue))) {
...
static gboolean disconnect_timeout(gpointer data)
{
	...
	notify_pending_commands_of_timeout(attrib);
	attrib->stale = TRUE;
