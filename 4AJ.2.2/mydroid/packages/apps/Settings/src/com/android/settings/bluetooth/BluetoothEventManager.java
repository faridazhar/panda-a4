...
import android.os.SystemProperties; // BLUETI ENHANCEMENT
        ...
        public void onReceive(Context context, Intent intent,
            ...
            if (cachedDevice == null) {
                ...
                if (SystemProperties.BLUETI_ENHANCEMENT) {
                    int deviceType = intent.getIntExtra(BluetoothDevice.EXTRA_DEVICE_TYPE,BluetoothDevice.DEVICE_TYPE_BR_EDR);
                    if (deviceType == BluetoothDevice.DEVICE_TYPE_LE) {
                        Log.d(TAG, "CachedBluetoothDeviceManager.onDeviceAppeared Setting device as LE "+device.getAddress());
                        cachedDevice.getDevice().SetDeviceLe();
                    } else {
                        Log.d(TAG, "CachedBluetoothDeviceManager.onDeviceAppeared Setting device as BR/EDR "+device.getAddress());
                        cachedDevice.getDevice().SetDeviceBrEdr();
                    }
                }
