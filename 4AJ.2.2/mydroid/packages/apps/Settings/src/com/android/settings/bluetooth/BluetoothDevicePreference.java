...
public final class BluetoothDevicePreference extends Preference implements
    ...
    private int getConnectionSummary() {
        ...
        boolean dunNotConnected = false;        // BLUETI ENHANCEMENT - DUN is not connected
        ...
        if (profileConnected) {
            if (a2dpNotConnected && headsetNotConnected) {
                ...
                if (dunNotConnected) {
                    return R.string.bluetooth_connected_no_dun;
                } else {
    ...
    private int getBtClassDrawable() {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            if (mCachedDevice.getDevice().GetDeviceType() == BluetoothDevice.DEVICE_TYPE_LE)
