...
import com.android.settings.R;       // BLUETI_ENHANCEMENT
...
final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    void onProfileStateChanged(LocalBluetoothProfile profile, int newProfileState) {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            checkIopDevice(profile, newProfileState);
        }
    ...
    private boolean updateProfiles() {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
                boolean isLeDevice =
                        (mDevice.GetDeviceType() == BluetoothDevice.DEVICE_TYPE_LE);
                Log.d(TAG, "updating profiles on device " + mDevice.getAddress() + " type " +
                        mDevice.GetDeviceType());
                mProfileManager.updateProfiles(uuids, localUuids, mProfiles, mRemovedProfiles,
                                                isLeDevice);
        } else {
                mProfileManager.updateProfiles(uuids, localUuids, mProfiles, mRemovedProfiles, false);
        }
    ...
    // BLUETI_ENHANCEMENT - Methode to check IOP devices.
    public void checkIopDevice(LocalBluetoothProfile profile, int newProfileState) {
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            if(newProfileState == BluetoothProfile.STATE_DISCONNECTED) {
                Log.d(TAG, "Check IOP for the disconnected device, name=" + mDevice.getName() +
                              " profile=" + profile.toString());
                if(profile.toString().equals("HID")) {
                    // Find HID devices that need to remove bonding after disconnection
                    String[] iopHidDevices = mContext.getResources().getStringArray(R.array.bluetooth_iop_hid);
                    for(String s : iopHidDevices) {
                        if(mDevice.getName().equals(s)) {
                            Log.d(TAG, "Unpair IOP HID device " + s + " when disconnected");
                            if (!mDevice.removeBond()) {
                                Log.d(TAG, "Can't remove bonding");
                            }
                        }
                    }
                }
