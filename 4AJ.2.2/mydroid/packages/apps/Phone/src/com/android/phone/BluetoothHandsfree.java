...
import android.provider.Settings; // BLUETI_ENHANCEMENT
...
public class BluetoothHandsfree {
    ...
    private static final int BRSF_AG_CODEC_NEGOTIATION = 1 << 9; // BLUETI_ENHANCEMENT
    ...
    private static final int BRSF_HF_CODEC_NEGOTIATION = 1 << 7; // BLUETI_ENHANCEMENT

    private static final int BAC_CODEC_CVSD = 1; // BLUETI_ENHANCEMENT
    private static final int BAC_CODEC_MSBC = 2; // BLUETI_ENHANCEMENT
    ...
    // Codec negotiation and selection - set default codec to CVSD
    private int mCurrentlySelectedCodec = BAC_CODEC_CVSD; // BLUETI_ENHANCEMENT
    private int mRemoteCodec; // BLUETI_ENHANCEMENT
    private boolean mCodecSelected; // BLUETI_ENHANCEMENT
    private boolean mCodecSelectionPending; // BLUETI_ENHANCEMENT
    private boolean mCodecConnectionPending; // BLUETI_ENHANCEMENT

    // Individual indicators activation
    private boolean mIndicatorServiceActivated; // BLUETI_ENHANCEMENT
    private boolean mIndicatorSignalActivated; // BLUETI_ENHANCEMENT
    private boolean mIndicatorRoamActivated; // BLUETI_ENHANCEMENT
    private boolean mIndicatorBattchgActivated; // BLUETI_ENHANCEMENT
    ...
    private class ScoSocketConnectThread extends Thread{
        ...
        public ScoSocketConnectThread(BluetoothDevice device) {
            try {
                if (SystemProperties.BLUETI_ENHANCEMENT) {
                    int codingFormat = (mCurrentlySelectedCodec == BAC_CODEC_MSBC ) ?
                        BluetoothAdapter.CODING_FORMAT_MSBC :
                        BluetoothAdapter.CODING_FORMAT_CVSD;
                    mOutgoingSco = device.createScoSocketEnhanced(codingFormat);
                } else {
    ...
    /* package */ synchronized void connectHeadset(HeadsetBase headset, int headsetType) {
        ...
        if (mHeadsetType == TYPE_HEADSET) {
            ...
            if (SystemProperties.BLUETI_ENHANCEMENT) {
                // By default all indicators are activated
                mIndicatorServiceActivated = true;
                mIndicatorSignalActivated = true;
                mIndicatorRoamActivated = true;
                mIndicatorBattchgActivated = true;

                mCodecSelected = false;
                mCodecSelectionPending = false;
                mCodecConnectionPending = false;
            }
    ...
    private class BluetoothPhoneState {
        ...
        private int gsmAsuToSignal(SignalStrength signalStrength) {
            ...
            if (SystemProperties.BLUETI_ENHANCEMENT) {
                if (VDBG) log("gsmAsuToSignal="+asu);
                if (asu == 99) asu = -1;
            }
            if      (asu >= 16) return 5;
        ...
        private synchronized void updateBatteryState(Intent intent) {
            ...
            if (mBattchg != batteryLevel) {
                ...
                if (SystemProperties.BLUETI_ENHANCEMENT) {
                    if (mIndicatorBattchgActivated == false) {
                        if (VDBG) log("Not sending change in deactivated indicator battchg: "
                                + mBattchg);
                        return;
                     }
                }
        ...
        private synchronized void updateSignalState(Intent intent) {
            ...
            if (signalStrength != null) {
                ...
                if (signal != mSignal) {
                    mSignal = signal;
                    if (SystemProperties.BLUETI_ENHANCEMENT) {
                        if (mIndicatorSignalActivated == false) {
                            if (VDBG) log("Not sending change in deactivated indicator signal: "
                                    + mSignal);
                            return;
                        }
                    }
        ...
        private synchronized void updateServiceState(boolean sendUpdate, ServiceState state) {
            ...
            if (service != mService) {
                ...
                if (sendUpdate) {
                    if (SystemProperties.BLUETI_ENHANCEMENT) {
                        if (mIndicatorServiceActivated == true) {
                            result.addResponse("+CIEV: 1," + mService);
                        } else {
                            if (VDBG) log("Not sending change in deactivated indicator service: " + mService);
                        }
                    } else {
                        result.addResponse("+CIEV: 1," + mService);
                    }
            ...
            if (roam != mRoam) {
                mRoam = roam;
                if (SystemProperties.BLUETI_ENHANCEMENT) {
                    if (mIndicatorRoamActivated == false) {
                        return;
                    }
                }
                if (sendUpdate) {
                    if (SystemProperties.BLUETI_ENHANCEMENT) {
                        if (mIndicatorRoamActivated == true) {
                            result.addResponse("+CIEV: 6," + mRoam);
                        } else {
                            if (VDBG) log("Not sending change in deactivated indicator roam: " + mRoam);
                        }
                    } else {
                        result.addResponse("+CIEV: 6," + mRoam);
                    }
        ...
        private AtCommandResult ring() {
            if (SystemProperties.BLUETI_ENHANCEMENT) {
                if (!mIgnoreRing && !mStopRing && mCM.getFirstActiveRingingCall().isRinging()) {
                    ...
                    // PTS TC/ICA/BV-07-I start
                    if (mServiceConnectionEstablished) {
                    // PTS TC/ICA/BV-07-I end
                    ...
                    // PTS TC/ICA/BV-07-I start
                        ...
                        Message msg = mStateChangeHandler.obtainMessage(RING);
                        mStateChangeHandler.sendMessageDelayed(msg, 1500);
                        return null;
                    }
                    // PTS TC/ICA/BV-07-I end
                }
        ...
        private synchronized void fastUpdateForCallDisconnect() {
            if (SystemProperties.BLUETI_ENHANCEMENT) {
                // PTS TC_AG_TCA_BV_04_I   -->
                if(sendUpdate()) {
                    if(mCall == 1) {
                        mCall = 0;
                        sendURC("+CIEV: 2," + mCall);
                    } else if(mCallsetup != 0) {
                        mCallsetup = 0;
                        sendURC("+CIEV: 3," + mCallsetup);
                    }
                }
                ...
                Log.e(TAG, "Method fastUpdateForCallDisconnect() is not supported");
            }
        }
    };
    ...
    /* package */ synchronized void onCallDisconnected(Connection.DisconnectCause cause) {
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            // PTS TC_AG_TCA_BV_04_I
            Log.e(TAG,"onCallDisconnected()");
            mBluetoothPhoneState.fastUpdateForCallDisconnect();
            // PTS TC_AG_TCA_BV_04_I
        } else {
            Log.e(TAG, "Method onCallDisconnected() is not supported");
        }
    }
    ...
    private void initializeHandsfreeAtParser() {
                ...
                if (isVirtualCallInProgress()) {
                    terminateScoUsingVirtualVoiceCall();
                        ...
                        if (mCM.hasActiveFgCall()) {
                            if (SystemProperties.BLUETI_ENHANCEMENT) {
                                // PTS TC_AG_TCA_BV_05_I -->
                                if(mCM.hasActiveRingingCall() && !mCM.hasActiveBgCall()) {
                                    PhoneUtils.hangup(mCM.getActiveFgCall().getEarliestConnection());
                                } else {
                ...
                if (args.length < 4) {
                    ...
                    if (valid) {
                        if ((mRemoteBrsf & BRSF_HF_CW_THREE_WAY_CALLING) == 0x0) {
                            ...
                            if (SystemProperties.BLUETI_ENHANCEMENT) {
                                if (mCodecSelectionPending == true) {
                                    // AT+BAC was received during SLC establishing, start codec setup
                                    // procedure
                                    sendURC("+BCS: " + mCurrentlySelectedCodec);
                                } else {
                                    // Check whether we need to return host's and chip's codecs configurations
                                    // to CVSD if it was configured to mSBC
                                    if (isCodecsReconfigToCvsdNeeded()) {
                                        mCurrentlySelectedCodec = BAC_CODEC_CVSD;
                                    }
                                }
                            }
                            if (isIncallAudio()) {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            ...
            parser.register("+BAC", new AtCommandHandler() {
                @Override
                public AtCommandResult handleSetCommand(Object[] args) {
                    // AT+BAC=<handsfree available codecs>
                    // Handsfree is telling us which codecs it supports.
                    // We respond with OK.
                    mRemoteCodec = BAC_CODEC_CVSD; // Set default to CVSD
                    if (args.length < 1) {
                        Log.w(TAG, "HF shall support at least CVSD codec");
                        return new AtCommandResult(AtCommandResult.ERROR);
                    } else if (args.length == 1) {
                        if (args[0].equals(BAC_CODEC_CVSD)) {
                            mRemoteCodec = BAC_CODEC_CVSD;
                        } else {
                            Log.w(TAG, "HF shall support at least CVSD codec");
                            return new AtCommandResult(AtCommandResult.ERROR);
                        }
                        ...
                        for (i = 0; i < args.length; i++) {
                            if (args[i].equals(BAC_CODEC_MSBC)) {
                                mRemoteCodec = BAC_CODEC_MSBC;
                                break;
                            }
                        }
                        if (i == args.length) {
                            Log.w(TAG, "HF doesn't support mandatory mSBC codec -select CVSD");
                        }
                    }

                    if ((mCodecSelected == false) || (mCurrentlySelectedCodec != mRemoteCodec)) {
                        if (mCodecSelectionPending == true) {
                            // Prevent loop of codec selection
                            mCurrentlySelectedCodec = BAC_CODEC_CVSD;
                        } else {
                            mCodecSelectionPending = true;
                            mCurrentlySelectedCodec = mRemoteCodec;
                        }
                        mCodecSelected = false;
                        sendURC("OK");  //Send reply first
                        // If AT+BAC was received during SLC establishing, start codec setup
                        // procedure later
                        if (mServiceConnectionEstablished == true) {
                            sendURC("+BCS: " + mCurrentlySelectedCodec); // Restart codec setup procedure
                        }
                        return new AtCommandResult(AtCommandResult.UNSOLICITED);
                    } else {
                        return new AtCommandResult(AtCommandResult.OK);
            ...
            parser.register("+BCC", new AtCommandHandler() {
                ...
                public AtCommandResult handleActionCommand() {
                    // AT+BCC
                    // Handsfree is requesting to start codec connection procedure
                    mUserWantsAudio = true;
                    if (mCodecSelected == true) {
                        mCodecConnectionPending = false;
                        // Start audio connection with previously selected codec
                        sendURC("OK");  //Send reply first
                        audioOn();
                    } else {
                        mCodecConnectionPending = true;
                        sendURC("OK");  //Send reply first
                        sendURC("+BCS: " + mCurrentlySelectedCodec); // Start codec setup procedure
                    }
                    return new AtCommandResult(AtCommandResult.UNSOLICITED);
               }
            });
            ...
            parser.register("+BCS", new AtCommandHandler() {
                ...
                public AtCommandResult handleSetCommand(Object[] args) {
                    // AT+BCS=<codec>
                    // Handsfree confirms/declines selected codec
                    if (args.length == 1 && (args[0] instanceof Integer)) {
                        mRemoteCodec = (Integer) args[0];
                        if (mRemoteCodec == mCurrentlySelectedCodec) {
                            // Codec is confirmed
                            mCodecSelected = true;
                            mCodecSelectionPending = false;
                            sendURC("OK");
                            ...
                            // Start audio connection if codec connection was requested by
                            // handsfree with AT+BCC
                            if (mCodecConnectionPending == true) {
                                mCodecConnectionPending = false;
                                audioOn();
                            } else if (isIncallAudio()) {
                                audioOn();
                            }
                            return new AtCommandResult(AtCommandResult.UNSOLICITED);
                        } else {
                            // Codec is declined -new AT+BAC command will restart negotiation
                            return new AtCommandResult(AtCommandResult.ERROR);
                        }
                    } else {
                        return new AtCommandResult(AtCommandResult.ERROR);
            ...
            parser.register("+BIA", new AtCommandHandler() {
                @Override
                public AtCommandResult handleSetCommand(Object[] args) {
                    // AT+BIA=<ind1, ind2...indN>
                    // Handsfree activates/deactivates individual indicators
                    // Indicators reported by our CIND response are in the following order:
                    // service, call, callsetup, callheld, signal, roam, battchg
                    // Indicators call, callsetup, callheld are mandatory and always active
                    if (args.length >= 1) {
                        if ((args[0] instanceof Integer) && ((Integer) args[0] == 1)) {
                            mIndicatorServiceActivated = true;
                        } else {
                            mIndicatorServiceActivated = false;
                        }
                        if ((args.length >= 5) && (args[4] instanceof Integer) &&
                                ((Integer) args[4] == 1)) {
                            mIndicatorSignalActivated = true;
                        } else {
                            mIndicatorSignalActivated = false;
                        }
                        if ((args.length >= 6) && (args[5] instanceof Integer) &&
                                ((Integer) args[5] == 1)) {
                            mIndicatorRoamActivated = true;
                        } else {
                            mIndicatorRoamActivated = false;
                        }
                        if ((args.length >= 7) && (args[6] instanceof Integer) &&
                                ((Integer) args[6] == 1)) {
                            mIndicatorBattchgActivated = true;
                        } else {
                            mIndicatorBattchgActivated = false;
                        }
                        return new AtCommandResult(AtCommandResult.OK);
                    } else {
                        return new AtCommandResult(AtCommandResult.ERROR);
    ...
    private boolean isCodecsReconfigToCvsdNeeded() {
        if (VDBG) log("isCodecsReconfigToCvsdNeeded");
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            // In case WBS is not supported but codecs were configured to mSBC, there is a
            // need to reconfigure them to CVSD
            if (((mRemoteBrsf & BRSF_HF_CODEC_NEGOTIATION) == 0x0) &&
                        (mCurrentlySelectedCodec == BAC_CODEC_MSBC)) {
                // Current remote device does not support WBS, as a previous one, hence,
                // need to reconfigure codecs to CVSD
                return true;
            } else {
                return false;
            }
        } else {
            Log.e(TAG, "Method isCodecsReconfigToCvsdNeeded() is not supported");
            return false;
        }
    }
