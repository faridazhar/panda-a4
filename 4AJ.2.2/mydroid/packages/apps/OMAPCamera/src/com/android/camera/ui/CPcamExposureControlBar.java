...
/**
 * A view that contains camera Exposure control and its layout.
 */
public class CPcamExposureControlBar extends CPcamExposureControl {
    @SuppressWarnings("unused")
    private static final String TAG = "ExposureControlBar";
    private static final int THRESHOLD_FIRST_MOVE = Util.dpToPixel(10); // pixels
    // Space between indicator icon and the Exposure-in/out icon.
    ...
    // The width of the Exposure control bar (including the '+', '-' icons and the
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        if (getResources().getConfiguration().orientation
                == Configuration.ORIENTATION_LANDSCAPE) {
            mSize = h;
            mIconSize = mExposureIn.getMeasuredHeight();
        } else {
            mSize = w;
            mIconSize = mExposureIn.getMeasuredWidth();
        }
    ...
    protected void onLayout(
        ...
        if (isLandscape) {
            if (mOrientation == 180) {
                mExposureOut.layout(0, 0, size, mIconSize);
                mExposureIn.layout(0, mSize - mIconSize, size, mSize);
                pos = mBar.getTop() + sliderPosition;
            } else {
                mExposureIn.layout(0, 0, size, mIconSize);
                mExposureOut.layout(0, mSize - mIconSize, size, mSize);
                pos = mBar.getBottom() - sliderPosition;
            }
            int sliderHeight = mExposureSlider.getMeasuredHeight();
            mExposureSlider.layout(0, (pos - sliderHeight / 2), size,
                    (pos + sliderHeight / 2));
        } else {
            if (mOrientation == 90) {
                mExposureIn.layout(0, 0, mIconSize, size);
                mExposureOut.layout(mSize - mIconSize, 0, mSize, size);
                pos = mBar.getRight() - sliderPosition;
            } else {
                mExposureOut.layout(0, 0, mIconSize, size);
                mExposureIn.layout(mSize - mIconSize, 0, mSize, size);
