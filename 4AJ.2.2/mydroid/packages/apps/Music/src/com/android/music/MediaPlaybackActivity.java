    ...
    View.OnTouchListener, View.OnLongClickListener
{
        ...
        public void onProgressChanged(SeekBar bar, int progress, boolean fromuser) {
            ...
            if ((now - mLastSeekEventTime) > 250) {
                ...
                try {
                    ...
                    if (BLUETI_ENHANCEMENT) {
                        mService.setPlaybackStatus(mService.isPlaying() ?
        ...
        public void onClick(View v) {
            ...
            try {
                if (mService.position() < 2000) {
                    ...
                    mService.seek(0);
                    if (BLUETI_ENHANCEMENT) {
                        mService.setPlaybackStatus(MEDIA_PLAYBACK_STATUS_PLAYING);
                    }
                    mService.play();
                }
            } catch (RemoteException ex) {
    ...
    public void onStart() {
        ...
        if (BLUETI_ENHANCEMENT) {
            f.addAction(MediaPlaybackService.REFRESH);
            f.addAction(MediaPlaybackService.SETTING_CHANGED);
        }
...
*/
        ...
        try {
            if(repcnt == 0) {
                ...
                if (((delta - mLastSeekEventTime) > 250) || repcnt < 0){
                    ...
                    if (BLUETI_ENHANCEMENT) {
                        mService.setPlaybackStatus(MEDIA_PLAYBACK_STATUS_REVERSE_SEEK);
                    }
                }
                if (repcnt >= 0) {
                    mPosOverride = newpos;
        ...
        try {
            if(repcnt == 0) {
                ...
                if (((delta - mLastSeekEventTime) > 250) || repcnt < 0){
                    ...
                    mLastSeekEventTime = delta;
                    if (BLUETI_ENHANCEMENT) {
                        mService.setPlaybackStatus(MEDIA_PLAYBACK_STATUS_FORWARD_SEEK);
                    }
                }
                if (repcnt >= 0) {
                    mPosOverride = newpos;
    ...
    private BroadcastReceiver mStatusListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(MediaPlaybackService.META_CHANGED)) {
                ...
                if (BLUETI_ENHANCEMENT) {
                    if (action.equals(MediaPlaybackService.REFRESH))
                        queueNextRefresh(1);
                    else if (action.equals(MediaPlaybackService.SETTING_CHANGED)) {
                        setShuffleButtonImage();
                        setRepeatButtonImage();
                    }
                }
    ...
    private static boolean getBluetiEnhancementProperty() {
        ...
        try {
            ...
            // Else, the code will work with BLUETI_ENHANCEMENT=false.
