        ...
        jobject o, jbyteArray pollBytes, jbyteArray actBytes)
{
    ...
    if (pollBytes != NULL && (e->GetArrayLength(pollBytes) >= 2) &&
        ...
        if (act[0] == 0x20 && poll[1] == 0x03) {
            //MIFARE DESFire EV1 GetVersion command 1
...
#ifdef NFC_JNI_TI_DEVICE
                    /* Complete the Desfire card get version sequence, which consists
                       of 3 commands in total. The response is not important. */

                    //MIFARE DESFire EV1 GetVersion command 2
                    uint8_t cmd1[] = {0x90, 0xAF, 0x00, 0x00, 0x00};

                    // Identifies as DESfire, use get version cmd to be sure
                    e->SetByteArrayRegion(versionCmd, 0, 5, (jbyte*)cmd1);
                    jbyteArray respBytes1 = com_android_nfc_NativeNfcTag_doTransceive(e, o,
                                versionCmd, JNI_TRUE, NULL);

                    //MIFARE DESFire EV1 GetVersion command 3
                    uint8_t cmd2[] = {0x90, 0xAF, 0x00, 0x00, 0x00};

                    // Identifies as DESfire, use get version cmd to be sure
                    e->SetByteArrayRegion(versionCmd, 0, 5, (jbyte*)cmd2);
                    jbyteArray respBytes2 = com_android_nfc_NativeNfcTag_doTransceive(e, o,
                                versionCmd, JNI_TRUE, NULL);
#endif
                }
