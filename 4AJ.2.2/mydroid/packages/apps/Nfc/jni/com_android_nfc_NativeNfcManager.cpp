...
#else
        status = phLibNfc_Mgt_DeInitialize(gHWRef, nfc_jni_deinit_download_callback, (void *)&cb_data, 0);
#endif
        REENTRANCE_UNLOCK();
...
static int nfc_jni_initialize(struct nfc_jni_native_data *nat) {
   ...
   if(!driverConfigured)
   {
       ...
       {
           ALOGE("nfc_jni_configure_driver() failed");
           goto clean_and_return;
       }
   }
...
#else
static jboolean com_android_nfc_NfcManager_deinitialize(JNIEnv *e, jobject o, jint mode)
#endif
{
   struct timespec ts;
   NFCSTATUS status;
...
#else
      status = phLibNfc_Mgt_DeInitialize(gHWRef, nfc_jni_deinit_callback, (void *)&cb_data, mode);
#endif
      REENTRANCE_UNLOCK();
      if (status == NFCSTATUS_PENDING)
      {
