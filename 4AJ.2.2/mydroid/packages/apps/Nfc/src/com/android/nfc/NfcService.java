...
public class NfcService extends Application implements DeviceHostListener {
    ...
    static final int TASK_DISABLE_BAT_LOW = 5;	// NFCTI_ENHANCEMENT
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        protected Void doInBackground(Integer... params) {
            ...
            switch (params[0].intValue()) {
                case TASK_ENABLE:
                    ...
                    break;
                case TASK_DISABLE:
                if (!SystemProperties.NFCTI_ENHANCEMENT) {
                    disableInternal();
                    break;
                } else {
                    disableInternal(0);	// 0 = NFC OFF (normal behaviour)
                    break;
                }
                case TASK_DISABLE_BAT_LOW:	// NFCTI_ENHANCEMENT
                if (SystemProperties.NFCTI_ENHANCEMENT) {
                    disableInternal(1);	// 1 = BAT LOW
                }
                break;
                case TASK_BOOT:
        ...
        boolean disableInternal(int mode) 	// NFCTI_ENHANCEMENT
        {
            if (mState == NfcAdapter.STATE_OFF) {
                return true;
            }
            ...
            mNfcDispatcher.setForegroundDispatch(null, null, null);
            boolean result = mDeviceHost.deinitialize(mode);	// NFCTI_ENHANCEMENT
            if (DBG) Log.d(TAG, "mDeviceHost.deinitialize() = " + result);
            ...
            if (handle == 0) {
                ...
                if (tempEnable) {
                    if (!SystemProperties.NFCTI_ENHANCEMENT) {
                        ...
                        disableInternal(0);	// 0 = NFC OFF (normal behaviour), NFCTI_ENHANCEMENT
                    }
                }
                return;
            }
    ...
    final class NfcAdapterService extends INfcAdapter.Stub {
        ...
        public boolean disable(boolean saveState) throws RemoteException {
            ...
            if (!SystemProperties.NFCTI_ENHANCEMENT) {
                ...
                if (saveState)	// Settings NFC OFF
                    new EnableDisableTask().execute(TASK_DISABLE);
                else		// Shutdown thread => BAT LOW
                    new EnableDisableTask().execute(TASK_DISABLE_BAT_LOW);
            }
            return true;
        }
