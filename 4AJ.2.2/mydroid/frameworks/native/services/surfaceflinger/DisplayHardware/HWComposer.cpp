...
int HWComposer::extendedApiLayerData(hwc_layer_extended_t* linfo) {
    uint32_t idx = linfo->idx;
    if (idx >= mListExt->numHwLayers)
        return -1;
    *linfo = mListExt->hwLayers[idx];
    linfo->idx = idx;
    return 0;
}
#endif
...
status_t HWComposer::createWorkList(size_t numLayers) {
    if (mHwc) {
        if (!mList || mCapacity < numLayers) {
            ...
            mListExt = (hwc_layer_list_extended_t*)malloc(size);
#endif
        }
        mList->flags = HWC_GEOMETRY_CHANGED;
#ifdef OMAP_ENHANCEMENT
        mList->flags |= HWC_EXTENDED_API;
        mListExt->numHwLayers = numLayers;
#endif
...
#ifdef OMAP_ENHANCEMENT
hwc_layer_extended_t* HWComposer::getLayersExt() const {
    return mListExt ? mListExt->hwLayers : 0;
}
#endif
