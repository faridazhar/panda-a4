...
public class AudioService extends IAudioService.Stub implements OnFinished {
   ...
   /** @hide Maximum volume index values for audio streams */
    private final int[] MAX_STREAM_VOLUME = new int[] {
        ...
        15, // STREAM_DTMF
        15,  // STREAM_TTS
        15  // STREAM_BLUETOOTH_AVRCP1_4 BLUETI_ENHANCEMENT
    };
    ...
    private final int[] STREAM_VOLUME_ALIAS = new int[] {
        ...
        AudioSystem.STREAM_BLUETOOTH_SCO    //STREAM_BLUETOOTH_AVRCP1_4 BLUETI_ENHANCEMENT
    };
    ...
    private final String[] STREAM_NAMES = new String[] {
            ...
            "STREAM_DTMF",
            "STREAM_TTS",
            "STREAM_BLUETOOTH_AVRCP1_4"    //BLUETI_ENHANCEMENT
    };
    ...
    public void adjustStreamVolume(int streamType, int direction, int flags) {
        ...
        if(streamType == AudioSystem.STREAM_BLUETOOTH_AVRCP1_4 && direction == 0)
            return;
    ...
    private int getActiveStreamType(int suggestedStreamType) {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            if(AudioManager.avrcp1_4Active())
                return AudioSystem.STREAM_BLUETOOTH_AVRCP1_4;
        }
        if (mVoiceCapable) {
            if (isInCommunication()) {
    ...
    int mBecomingNoisyIntentDevices =
            ...
            AudioSystem.DEVICE_OUT_ALL_A2DP | AudioSystem.DEVICE_OUT_ANLG_DOCK_HEADSET;
    ...
    private Stack<RemoteControlStackEntry> mMPStack = new Stack<RemoteControlStackEntry>();  // BLUETI_ENHANCEMENT

    public void registerMediaPlayerEventReceiver(ComponentName eventReceiver) {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            Log.i(TAG, "  Current Media  registerMediaPlayerEventReceiver() for " + eventReceiver);

            synchronized(mMPStack){
                if (mMPStack.empty() || !mMPStack.peek().mReceiverComponent.equals(eventReceiver)) {
                    RemoteControlStackEntry rcse = new RemoteControlStackEntry(eventReceiver);

                    mMPStack.remove(rcse);
                    mMPStack.push(rcse);
                }
            }
        }
    ...
    public void unregisterMediaPlayerEventReceiver(ComponentName eventReceiver) {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            Log.i(TAG, "  Current Media  unregisterCurrentMediaEventReceiver() for " + eventReceiver);
    ...
    public void setCurrentMedia(Bundle metadata) {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            Intent intent = new Intent(AudioManager.ACTION_MEDIA_CHANGED);
    ...
    public void setCurrentMediaPlaybackStatus(int status, long elapsed) {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            Intent intent = new Intent(AudioManager.ACTION_MEDIA_PLAYBACK_CHANGED);
            intent.putExtra(AudioManager.EXTRA_MEDIA_PLAYBACK_STATUS, status);
            intent.putExtra(AudioManager.EXTRA_MEDIA_PLAYBACK_ELAPSED, elapsed);
            mContext.sendStickyBroadcast(intent);
        }
    }
    ...
    public void setCurrentMediaPlayerSettings(Bundle settings) {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            Intent intent = new Intent(AudioManager.ACTION_MEDIA_PLAYER_SETTINGS_CHANGED);
