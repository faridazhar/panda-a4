...
#define DBUS_SERVICE_IFACE BLUEZ_DBUS_BASE_IFC ".Service"
#define DBUS_DUN_GATEWAY_IFACE BLUEZ_DBUS_BASE_IFC ".DunGateway"
...
#ifdef BLUETI_ENHANCEMENT
static jboolean setDevicePropertySyncNative(JNIEnv *env, jobject object, jstring path,
                                               jstring key, void *value, jint type) {
#ifdef HAVE_BLUETOOTH
    LOGV("%s", __FUNCTION__);
    native_data_t *nat = get_native_data(env, object);
...
#ifdef BLUETI_ENHANCEMENT
static jboolean setDevicePropertyBooleanSyncNative(JNIEnv *env, jobject object,
                                                     jstring path, jstring key, jint value) {
#ifdef HAVE_BLUETOOTH
    return setDevicePropertySyncNative(env, object, path, key,
                                        (void *)&value, DBUS_TYPE_BOOLEAN);
#else
    return JNI_FALSE;
#endif
...
#ifdef BLUETI_ENHANCEMENT
static jint addServiceRecordXMLNative(JNIEnv *env, jobject object,jstring xml) {
    LOGI("%s", __FUNCTION__);
#ifdef HAVE_BLUETOOTH
    native_data_t *nat = get_native_data(env, object);
    if (nat) {
        const char *c_xml = env->GetStringUTFChars(xml, NULL);
        LOGI("... xml = %s", c_xml);
        DBusMessage *reply = dbus_func_args(env, nat->conn,
                           get_adapter_path(env, object),
                           DBUS_SERVICE_IFACE, "AddRecord",
                           DBUS_TYPE_STRING, &c_xml,
                           DBUS_TYPE_INVALID);
        env->ReleaseStringUTFChars(xml, c_xml);
...
static jboolean removeServiceRecordXMLNative(JNIEnv *env, jobject object,jint handle) {
    LOGI("%s", __FUNCTION__);
#ifdef HAVE_BLUETOOTH
    native_data_t *nat = get_native_data(env, object);
    if (nat) {
        ...
        DBusMessage *reply = dbus_func_args(env, nat->conn,
                           get_adapter_path(env, object),
                           DBUS_SERVICE_IFACE, "RemoveRecord",
...
#ifdef BLUETI_ENHANCEMENT
static jboolean enableBluetoothDunNative(JNIEnv *env, jobject object,
                                         jboolean value) {
    ALOGV("%s", __FUNCTION__);
#ifdef HAVE_BLUETOOTH
    native_data_t *nat = get_native_data(env, object);
    if (nat) {
        DBusMessage *reply;
        if (value) {
            ALOGI("enableBluetoothDunNative true");
            reply = dbus_func_args(env, nat->conn,
                                  get_adapter_path(env, object),
                                  DBUS_DUN_GATEWAY_IFACE,
                                  "Enable",
                                  DBUS_TYPE_INVALID);
        } else {
            ALOGI("enableBluetoothDunNative false");
            reply = dbus_func_args(env, nat->conn,
                                  get_adapter_path(env, object),
                                  DBUS_DUN_GATEWAY_IFACE,
                                  "Disable",
...
static jboolean disconnectDunDeviceNative(JNIEnv *env, jobject object) {
    ALOGV("%s", __FUNCTION__);
#ifdef HAVE_BLUETOOTH
    jboolean ret = JNI_FALSE;
    native_data_t *nat = get_native_data(env, object);
    ALOGI("disconnectDunDeviceNative");
    if (nat) {
        ...
        msg = dbus_message_new_method_call(BLUEZ_DBUS_BASE_IFC,
                                           ...
                                           "DisconnectDevice");
        if (!msg) {
            ALOGE("%s: Can't allocate new method call for DUN disconnect!", __FUNCTION__);
            return JNI_FALSE;
        }
...
#ifdef BLUETI_ENHANCEMENT
static jboolean readRssiLevelNative(JNIEnv *env, jobject object, jstring devicePath) {
    LOGV("%s", __FUNCTION__);
#ifdef HAVE_BLUETOOTH
    native_data_t *nat = get_native_data(env, object);
    if (nat) {
        const char *c_path = env->GetStringUTFChars(devicePath, NULL);
        DBusError err;
        dbus_error_init(&err);

        DBusMessage *reply = dbus_func_args(env, nat->conn,
                                            c_path,
                                            DBUS_DEVICE_IFACE, "GetRssiLevel",
                                            DBUS_TYPE_INVALID);
        env->ReleaseStringUTFChars(devicePath, c_path);
...
static jboolean disconnectDeviceNative(JNIEnv *env, jobject object, jstring path) {
#ifdef HAVE_BLUETOOTH
    LOGV("%s", __FUNCTION__);
    native_data_t *nat = get_native_data(env, object);
    if (nat) {
        ...
        msg = dbus_message_new_method_call(BLUEZ_DBUS_BASE_IFC,
                                          c_path, DBUS_DEVICE_IFACE, "Disconnect");
        if (!msg) {
            LOGE("%s: Can't allocate new method call for device Disconnect!", __FUNCTION__);
            env->ReleaseStringUTFChars(path, c_path);
            return JNI_FALSE;
        }
...
static jboolean connectAttDeviceNative(JNIEnv *env,
                                        jobject object,
                                        jstring address) {
#ifdef HAVE_BLUETOOTH
    LOGV("%s", __FUNCTION__);
    ...
    msg = dbus_message_new_method_call(BLUEZ_DBUS_BASE_IFC,
                                       ...
                                       "ConnectLowEnergy");
    if (!msg) {
        LOGE("%s: Can't allocate new method call for LE connect!", __FUNCTION__);
        env->ReleaseStringUTFChars(address, c_address);
        return JNI_FALSE;
    }

    dbus_message_append_args(msg, DBUS_TYPE_STRING, &c_address, DBUS_TYPE_INVALID);
...
static jboolean clearLeWhiteListNative(JNIEnv *env, jobject object) {
#ifdef HAVE_BLUETOOTH
    LOGV("%s", __FUNCTION__);
    jboolean ret = JNI_FALSE;
    native_data_t *nat = get_native_data(env, object);
    ...
    msg = dbus_message_new_method_call(BLUEZ_DBUS_BASE_IFC,
                                       ...
                                       "ClearLeWhiteList");
    if (!msg) {
        LOGE("%s: Can't allocate new method call for LE WL clear!", __FUNCTION__);
        return JNI_FALSE;
    }
...
static jboolean startWhiteListScanNative(JNIEnv *env, jobject object, jboolean fast) {
#ifdef HAVE_BLUETOOTH
    LOGV("%s", __FUNCTION__);
    jboolean ret = JNI_FALSE;
    native_data_t *nat = get_native_data(env, object);
    ...
    msg = dbus_message_new_method_call(BLUEZ_DBUS_BASE_IFC,
                                       ...
                                       "StartWhiteListScan");
    if (!msg) {
        LOGE("%s: Can't allocate new method call for start WL scan!", __FUNCTION__);
        return JNI_FALSE;
    }

    dbus_message_append_args(msg, DBUS_TYPE_BOOLEAN, &fast, DBUS_TYPE_INVALID);
...
static jboolean stopWhiteListScanNative(JNIEnv *env, jobject object) {
#ifdef HAVE_BLUETOOTH
    LOGV("%s", __FUNCTION__);
    jboolean ret = JNI_FALSE;
    native_data_t *nat = get_native_data(env, object);
...
#ifdef BLUETI_ENHANCEMENT
    {"setDevicePropertyBooleanSyncNative", "(Ljava/lang/String;Ljava/lang/String;I)Z",
            (void *)setDevicePropertyBooleanSyncNative},
#endif /*BLUETI_ENHANCEMENT*/
...
#ifdef BLUETI_ENHANCEMENT
    {"enableBluetoothDunNative", "(Z)Z", (void *)enableBluetoothDunNative},
    {"disconnectDunDeviceNative", "()Z", (void *)disconnectDunDeviceNative},
#endif /*BLUETI_ENHANCEMENT*/
    // Health function
...
#ifdef BLUETI_ENHANCEMENT
    {"readRssiLevelNative", "(Ljava/lang/String;)Z", (void *)readRssiLevelNative},
    {"addServiceRecordXMLNative", "(Ljava/lang/String;)I", (void *)addServiceRecordXMLNative},
    {"removeServiceRecordXMLNative", "(I)Z", (void *)removeServiceRecordXMLNative},

    {"disconnectDeviceNative", "(Ljava/lang/String;)Z", (void *)disconnectDeviceNative},
    {"connectAttDeviceNative", "(Ljava/lang/String;)Z", (void *)connectAttDeviceNative},
    {"clearLeWhiteListNative", "()Z", (void *)clearLeWhiteListNative},
    {"startWhiteListScanNative", "(Z)Z", (void *)startWhiteListScanNative},
    {"stopWhiteListScanNative", "()Z", (void *)stopWhiteListScanNative},
#endif /*BLUETI_ENHANCEMENT*/
};
