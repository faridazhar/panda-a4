...
static jfieldID  field_mCodingFormat; /* read-only */
...
#ifdef BLUETI_ENHANCEMENT
    case TYPE_SCO:
        codingFormat = env->GetIntField(obj,field_mCodingFormat);
        lm = codingFormat;
        break;
#endif /*BLUETI_ENHANCEMENT*/
    }
...
#ifdef BLUETI_ENHANCEMENT
         if(type == TYPE_SCO) {
             if (setsockopt(fd, SOL_SCO, SCO_CODING_FORMAT, &lm, sizeof(lm))) {
                LOGV("setsockopt(SCO_CODING_FORMAT) failed, throwing");
                jniThrowIOException(env, errno);
                return;
             }
         } else
...
#ifdef BLUETI_ENHANCEMENT
    field_mCodingFormat = env->GetFieldID(clazz, "mCodingFormat", "I");
#endif /*BLUETI_ENHANCEMENT*/
