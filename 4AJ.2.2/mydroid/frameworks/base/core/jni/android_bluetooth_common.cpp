...
static Properties remote_device_properties[] = {
    ...
    {"Temporary", DBUS_TYPE_BOOLEAN},
    {"Type", DBUS_TYPE_UINT32},
    {"WhiteList", DBUS_TYPE_BOOLEAN},
    {"Vendor", DBUS_TYPE_UINT16},
    {"VendorSource", DBUS_TYPE_UINT16},
    {"Version",DBUS_TYPE_UINT16},
    {"Product",DBUS_TYPE_UINT16},
#endif /*BLUETI_ENHANCEMENT*/
#ifdef BLUETI_ENHANCEMENT
    {"Batteries", DBUS_TYPE_ARRAY},
#endif /*BLUETI_ENHANCEMENT*/
};
...
#ifdef BLUETI_ENHANCEMENT
static Properties battery_properties[] = {
    {"Namespace", DBUS_TYPE_BYTE},
    {"Level", DBUS_TYPE_BYTE},
    {"Description", DBUS_TYPE_UINT16},
};
#endif
...
int get_property(DBusMessageIter iter, Properties *properties,
    ...
    *prop_index = i;
    ...
    case DBUS_TYPE_UINT16:
        dbus_message_iter_get_basic(&prop_val, &uint_val);
        value->uint_val = uint_val;
        *len = 1;
        break;
    case DBUS_TYPE_BYTE:
        dbus_message_iter_get_basic(&prop_val, &byte_val);
        value->byte_val = byte_val;
        *len = 1;
        break;
#endif
...
void create_prop_array(JNIEnv *env, jobjectArray strArray, Properties *property,
    ...
    *array_index += 1;
    ...
    if (prop_type == DBUS_TYPE_UINT32 || prop_type == DBUS_TYPE_INT16) {
        ...
        *array_index += 1;
#ifdef BLUETI_ENHANCEMENT
    } else if (prop_type == DBUS_TYPE_UINT16) {
        sprintf(buf, "%d", value->uint_val);
        set_object_array_element(env, strArray, buf, *array_index);
        *array_index += 1;
    } else if (prop_type == DBUS_TYPE_BYTE) {
        sprintf(buf, "%d", value->byte_val);
        set_object_array_element(env, strArray, buf, *array_index);
        *array_index += 1;
#endif
...
#ifdef BLUETI_ENHANCEMENT
jobjectArray parse_battery_properties(JNIEnv *env, DBusMessageIter *iter) {
    return parse_properties(env, iter, (Properties *) &battery_properties,
                          sizeof(battery_properties) / sizeof(Properties));
}

jobjectArray parse_battery_property_change(JNIEnv *env, DBusMessage *msg) {
    return parse_property_change(env, msg, (Properties *) &battery_properties,
                    sizeof(battery_properties) / sizeof(Properties));
}
#endif
