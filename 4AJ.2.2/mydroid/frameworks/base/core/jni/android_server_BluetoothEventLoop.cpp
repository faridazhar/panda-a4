...
static jmethodID method_onDeviceRssiLevelresult;
static jmethodID method_onBatteryPropertyChanged;
static jmethodID method_onDunDeviceConnectionResult;
static jmethodID method_onDunDeviceDisconnected;
static jmethodID method_onDunDeviceConnected;
#endif /*BLUETI_ENHANCEMENT*/

typedef event_loop_native_data_t native_data_t;

#define EVENT_LOOP_REFS 25
...
#ifdef BLUETI_ENHANCEMENT
    method_onDeviceRssiLevelresult = env->GetMethodID(clazz, "onRssiLevelResult",
            "(Ljava/lang/String;S)V");
    method_onBatteryPropertyChanged = env->GetMethodID(clazz, "onBatteryPropertyChanged",
                                               "(Ljava/lang/String;[Ljava/lang/String;)V");
    method_onDunDeviceConnected = env->GetMethodID(clazz, "onDunDeviceConnected",
                                                     "(Ljava/lang/String;)V");
    method_onDunDeviceDisconnected = env->GetMethodID(clazz, "onDunDeviceDisconnected",
                                                              "(Ljava/lang/String;)V");
#endif /*BLUETI_ENHANCEMENT*/
...
static jboolean setUpEventLoop(native_data_t *nat) {
    ...
    if (nat != NULL && nat->conn != NULL) {
        ...
        dbus_bus_add_match(nat->conn,
                "type='signal',interface='"BLUEZ_DBUS_BASE_IFC".Battery'",
                &err);
        if (dbus_error_is_set(&err)) {
            LOG_AND_FREE_DBUS_ERROR(&err);
            return JNI_FALSE;
        ...
        dbus_bus_add_match(nat->conn,
                "type='signal',interface='"BLUEZ_DBUS_BASE_IFC".DunGateway'",
                &err);
        if (dbus_error_is_set(&err)) {
            LOG_AND_FREE_DBUS_ERROR(&err);
            return JNI_FALSE;
...
#ifdef BLUETI_ENHANCEMENT
extern DBusHandlerResult gattclient_event_filter(DBusMessage *msg, JNIEnv *env);
#endif /*BLUETI_ENHANCEMENT*/
...
static DBusHandlerResult event_filter(DBusConnection *conn, DBusMessage *msg,
    ...
    if (dbus_message_is_signal(msg,
        ...
        if (str_array != NULL) {
            ...
            if (!strncmp(c_property, "Connectable", strlen("Connectable"))) {
                jstring value =
       ...
       if (dbus_message_get_args(msg, &err,
           ...
           ALOGV("... address = %s", c_address);
           env->CallVoidMethod(nat->me,
                               method_onDunDeviceDisconnected,
                               env->NewStringUTF(c_address));
       } else {
           LOG_AND_FREE_DBUS_ERROR_WITH_MSG(&err, msg);
       }
       ...
       goto success;
        ...
        if (dbus_message_get_args(msg, &err,
            ...
            LOGI("got signal");
            env->CallVoidMethod(nat->me,
                method_onDeviceRssiLevelresult,
                                env->NewStringUTF(c_path),
                                rssi);
            LOGI("sent event");
        } else {
        ...
        goto success;
                                         ...
                                         "PropertyChanged")) {
        jobjectArray str_array =
                        parse_battery_property_change(env, msg);
        if (str_array != NULL) {
            const char *c_path = dbus_message_get_path(msg);
            env->CallVoidMethod(nat->me,
                                method_onBatteryPropertyChanged,
                                env->NewStringUTF(c_path),
                                str_array);
        } else {
           LOG_AND_FREE_DBUS_ERROR_WITH_MSG(&err, msg);
...
#ifdef BLUETI_ENHANCEMENT
    ret = gattclient_event_filter(msg, env);
