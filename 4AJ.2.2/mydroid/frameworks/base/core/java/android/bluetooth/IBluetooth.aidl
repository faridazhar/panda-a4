...
interface IBluetooth
{
    ...
    boolean isTemporary(in String address);  // BLUETI_ENHANCEMENT
    boolean setTrustExt(in String address, in int value);  // BLUETI_ENHANCEMENT
    ...
    boolean readRssi(String address);  // BLUETI_ENHANCEMENT
    int addServiceRecordXML(in String xml, in ParcelUuid uuid, int channel, IBinder b);// BLUETI_ENHANCEMENT

    // BLUETI_ENHANCEMENT- ATT server APIs
    boolean connectAttDevice(in BluetoothDevice device);
    boolean disconnectAttDevice(in BluetoothDevice device);
    int getAttDeviceConnectionState(in BluetoothDevice device);
    boolean setAttPriority(in BluetoothDevice device, int priority);
    int getAttPriority(in BluetoothDevice device);

    String getDeviceProperty(in String address, in String property);  // BLUETI_ENHANCEMENT
    String[] getBatteries(in String address);                    // BLUETI_ENHANCEMENT
    int getBatteryLevel(in String address, in String batteryId); // BLUETI_ENHANCEMENT

    // BLUETI_ENHANCEMENT - DUN profile APIs (Gateway)
    boolean isDunEnabled();
    boolean enableBluetoothDun(boolean value);
    boolean disconnectDunDevice();
    int getDunDeviceConnectionState(in BluetoothDevice device);
    boolean setDunDevicePriority(in BluetoothDevice device, int priority);
    int getDunDevicePriority(in BluetoothDevice device);
    List<BluetoothDevice> getConnectedDunDevices();
    List<BluetoothDevice> getDunDevicesMatchingConnectionStates(in int[] states);
}
