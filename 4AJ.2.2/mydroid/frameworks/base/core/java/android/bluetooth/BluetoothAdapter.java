...
public final class BluetoothAdapter {
    ...
    /**
     * Coding format of the BT chip's codec used for establishing synchronous connection (see BT SIG
     * HCI Assigned Numbers - values 6-254 reserved).
     */
    /** @hide */
    public static final int CODING_FORMAT_U_LAW = 0; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_A_LAW = 1; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_CVSD = 2; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_TRANSPARENT = 3; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_LINEAR_PCM = 4; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_MSBC = 5; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_RESERVED = 254; // BLUETI_ENHANCEMENT
    /** @hide */
    public static final int CODING_FORMAT_VENDOR_SPECIFIC = 255; // BLUETI_ENHANCEMENT
    ...
    private static class RfcommChannelPicker {
        ...
        private static final int[] BLUETI_RESERVED_RFCOMM_CHANNELS =  new int[] {
            21,  // FTPS
            22,  // DUN
        }; // BLUETI_ENHANCEMENT
            ...
            synchronized (RfcommChannelPicker.class) {
                if (sChannels == null) {
                    ...
                    if (SystemProperties.BLUETI_ENHANCEMENT) {
                        for (int reserved : BLUETI_RESERVED_RFCOMM_CHANNELS) {
                            sChannels.remove(new Integer(reserved));
                        }
                    }
                    sRandom = new Random();
     ...
     * @hide
     */
    public final static int BLUETOOTH_RFCOMM_AUTOMATIC_CHANNEL = -1; // BLUETI_ENHANCEMENT

    /**
     * Create a listening, insecure RFCOMM Bluetooth socket with Service Record.
     * The Service record is recived as an xml string according to the format
     * in org.Bluez.Service api. In the xml put "{RFCOMM_CHANNEL_PLACE_HOLDER}"
     * in the place of the rfcomm channel. It will be replaced with the
     * selected rfcomm channel.
     * @hide
     */
    public BluetoothServerSocket listenUsingInsecureRfcommWithServiceRecordXML(String name, String xml, UUID uuid)
            throws IOException {
        if (SystemProperties.BLUETI_ENHANCEMENT)
            return createNewRfcommSocketAndRecordXML(name,xml, uuid, false, false,
                                            BLUETOOTH_RFCOMM_AUTOMATIC_CHANNEL);
     ...
     * in org.Bluez.Service api. In the xml put "{RFCOMM_CHANNEL_PLACE_HOLDER}"
     * in the place of the rfcomm channel. It will be replaced with the
     * provided rfcomm channel. if RFCOMM channel is set to BLUETOOTH_RFCOMM_AUTOMATIC_CHANNEL,
     * RFCOMM channel will be assigned automatically out of the available channel pool.
     * @hide
     */
    public BluetoothServerSocket listenUsingInsecureRfcommWithServiceRecordXML(String name, String xml, UUID uuid, int channel)
            throws IOException {
        if (SystemProperties.BLUETI_ENHANCEMENT)
            return createNewRfcommSocketAndRecordXML(name,xml, uuid, false, false, channel);
    ...
    private BluetoothServerSocket createNewRfcommSocketAndRecordXML(String name, String xml, UUID uuid,
            boolean auth, boolean encrypt, int channel) throws IOException {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            ...
            int errno;
            boolean autoChannel = false;

            if (channel == BLUETOOTH_RFCOMM_AUTOMATIC_CHANNEL)
                autoChannel = true;
            ...
            xml = xml.replace("{RFCOMM_CHANNEL_PLACE_HOLDER}",new Integer(channel).toString());


            int handle = -1;
            try {
                handle = mService.addServiceRecordXML(xml, new ParcelUuid(uuid), channel,
                        new Binder());
            } catch (RemoteException e) {Log.e(TAG, "", e);}
            if (handle == -1) {
                try {
    ...
    public boolean getProfileProxy(Context context, BluetoothProfile.ServiceListener listener,
        ...
        if (profile == BluetoothProfile.HEADSET) {
            ...
            return true;
        } else if (SystemProperties.BLUETI_ENHANCEMENT) {
            if (profile == BluetoothProfile.ATT_SRV) {
            BluetoothAtt att = new BluetoothAtt(context, listener);
            return true;
    ...
    public void closeProfileProxy(int profile, BluetoothProfile proxy) {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            if (profile == BluetoothProfile.DUN) {
