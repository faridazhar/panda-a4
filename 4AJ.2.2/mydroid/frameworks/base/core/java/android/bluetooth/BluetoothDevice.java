...
import java.util.HashMap; // BLUETI_ENHANCEMENT
...
public final class BluetoothDevice implements Parcelable {
     ...
     * @hide
     */
    public static final int DEVICE_TYPE_BR_EDR    = 1;
    ...
    private int mDeviceType = DEVICE_TYPE_UNDEFINED;  // BLUETI_ENHANCEMENT
    ...
    public int GetDeviceType() {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            if (mDeviceType == DEVICE_TYPE_UNDEFINED) {
                String type = null;
                try {
                    type = sService.getDeviceProperty(mAddress, "Type");
                } catch (RemoteException e) {
                    Log.e(TAG, "", e);
                    return DEVICE_TYPE_UNDEFINED;
                }
     ...
     * @hide
     */
    public void SetDeviceBrEdr() {
        if(SystemProperties.BLUETI_ENHANCEMENT)
            mDeviceType = DEVICE_TYPE_BR_EDR;
    }
     ...
     * @hide
     */
    public void SetDeviceLe() {
        if(SystemProperties.BLUETI_ENHANCEMENT)
            mDeviceType = DEVICE_TYPE_LE;
    }
    ...
    public static final String DEVICE_RSSI =
            "android.bluetooth.device.action.DEVICE_RSSI";

    /**
     * BLUETI_ENHANCEMENT-
     * Used to identify device type in {@link #ACTION_FOUND}
     * @hide
     */
     ...
     * @hide
     */
    public static final String DEVICE_BATT_LEVEL =
            "android.bluetooth.device.action.DEVICE_BATT_LEVEL";

    /**
     * BLUETI_ENHANCEMENT-
     * Used to identify battery type in {@link #DEVICE_BATT_LEVEL}
     * @hide
     */
    public static final String EXTRA_BATT_ID = "android.bluetooth.device.extra.BATT_ID";

    /**
     * BLUETI_ENHANCEMENT-
     * Used to identify battery level in {@link #DEVICE_BATT_LEVEL}
     * @hide
     */
    public static final String EXTRA_BATT_LEVEL = "android.bluetooth.device.extra.BATT_LEVEL";
    ...
    /** Device Untrusted
     * @hide */
    public static final int TRUST_DEVICE_NONE = 0;  // BLUETI_ENHANCEMENT
    /** Trust the device until unpaired
     * @hide */
    public static final int TRUST_DEVICE_ALWAYS = 1;  // BLUETI_ENHANCEMENT
    /** Trust the device until it disconnects
     * @hide */
    public static final int TRUST_DEVICE_THIS_LINK = 2;  // BLUETI_ENHANCEMENT
    ...
    /**
     * BLUETI_ENHANCEMENT-
     * returns if this is a temporary device (well be unpaired when connection is over).
     * @return isTemporary
     * @hide
     */
    public boolean isTemporary() {
        ...
        {
            try {
                return sService.isTemporary(mAddress);
            } catch (RemoteException e) {
                Log.e(TAG, "", e);
            }
            return false;
    ...
    /**
     * Set trust state for a remote device (Enhanced).
     * @hide
     */
    public boolean setTrustExt(int value) {
        if(SystemProperties.BLUETI_ENHANCEMENT)
        {
            try {
                return sService.setTrustExt(mAddress, value);
            } catch (RemoteException e) {
                Log.e(TAG, "", e);
            }
            return false;
    ...
    public boolean setTrust(boolean value) {
        if (SystemProperties.BLUETI_ENHANCEMENT)
        {
            int trusted=BluetoothDevice.TRUST_DEVICE_NONE;
            if (value == true) {
                trusted = BluetoothDevice.TRUST_DEVICE_ALWAYS;
            }
            return setTrustExt(trusted);
        } else{
            try {
    ...
    /**
     * Construct a SCO socket for WBS support ready to start an outgoing connection.
     * Call #connect on the returned #BluetoothSocket to begin the connection.
     * <p>Requires {@link android.Manifest.permission#BLUETOOTH_ADMIN}
     *
     * @return a SCO BluetoothSocket
     ...
     * @hide
     */
    public BluetoothSocket createScoSocketEnhanced(int codingFormat) throws IOException {
        if(SystemProperties.BLUETI_ENHANCEMENT) {
            return new BluetoothSocket(BluetoothSocket.TYPE_SCO, -1, true, true, this, codingFormat);
        } else {
            Log.e(TAG, "Method createScoSocketEnhanced(codingFormat) is not supported");
            return null;
        }
    }
     ...
     */
     public boolean readRSSI() {
         if(SystemProperties.BLUETI_ENHANCEMENT) {
             try {
                return sService.readRssi(mAddress);
             } catch (RemoteException e) {Log.e(TAG, "", e);}
                return false;
             }
         return false;
     ...
     public String[] getBatteries() {
         if(SystemProperties.BLUETI_ENHANCEMENT) {
             try {
                 return sService.getBatteries(mAddress);
             } catch (RemoteException e) {Log.e(TAG, "", e);}
      ...
      * @hide
      */
     public int getBatteryLevel(String batteryId) {
         if(SystemProperties.BLUETI_ENHANCEMENT) {
             try {
                 return sService.getBatteryLevel(mAddress, batteryId);
             } catch (RemoteException e) {Log.e(TAG, "", e);}
