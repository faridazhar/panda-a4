...
class BluetoothEventLoop {
   ...
   //Pin Code request LCD Display Time
    private static final int PIN_CODE_REQUEST_WAKELOCK_TIME = 6000;  // BLUETI_ENHANCEMENT
    ...
    if (SystemProperties.BLUETI_ENHANCEMENT)
        mAdapter.getProfileProxy(mContext, mProfileServiceListener, BluetoothProfile.ATT_SRV);
    }
    ...
    private void addDevice(String address, String[] properties) {
        ...
        Boolean leDevice =  false;  // BLUETI_ENHANCEMENT
        ...
        String type = deviceProperties.getProperty(address, "Type"); // BLUETI_ENHANCEMENT
        short rssiValue;
        if (SystemProperties.BLUETI_ENHANCEMENT) {
        // LE random or public address
        if (type != null && Integer.parseInt(type) > 0) {
                classValue = "0";
                leDevice =  true;
            }

            Log.d(TAG,"Address:"+address+" RSSI:"+rssi+" name:"+name+" LE:"+leDevice + " Class:"+classValue + "Address Type:"+type);
        }
        ...
        if (classValue != null) {
            ...
            if (SystemProperties.BLUETI_ENHANCEMENT)
                intent.putExtra(BluetoothDevice.EXTRA_DEVICE_TYPE,
                    (leDevice ? BluetoothDevice.DEVICE_TYPE_LE : BluetoothDevice.DEVICE_TYPE_BR_EDR));
    ...
    /*package*/ void onPropertyChanged(String[] propValues) {
        ...
        if (name.equals("Name")) {
        ...
        } else if(name.equals("Connectable")){
            if (SystemProperties.BLUETI_ENHANCEMENT) {
                adapterProperties.setProperty(name, propValues[1]);

                if (name.equals("Connectable")) {
                    if (mBluetoothService.getBluetoothState() == BluetoothAdapter.STATE_TURNING_OFF) {
                        mBluetoothState.sendMessage(BluetoothAdapterStateMachine.POWER_STATE_CHANGED,new Boolean(false));
                        return;
                    } else {
                        mBluetoothState.sendMessage(BluetoothAdapterStateMachine.SCAN_MODE_CHANGED);
                    }
                ...
                String discoverable = name.equals("Discoverable") ? propValues[1] :
                    adapterProperties.getProperty("Discoverable");
                String connectable = name.equals("Connectable") ? propValues[1] :
                    adapterProperties.getProperty("Connectable");

                // This shouldn't happen, unless Adapter Properties are null.

                if (pairable == null || discoverable == null || connectable == null)
                    return;
            ...
            mContext.sendBroadcast(intent, BLUETOOTH_PERM);
        } else if (name.equals("Devices") || name.equals("UUIDs") || name.equals("Services") || name.equals("Batteries")) {
            String value = null;
    ...
    private void onDevicePropertyChanged(String deviceObjectPath, String[] propValues) {
        ...
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            if (name.equals("Temporary")) {
                if (DBG)
                    log("set temporary state succeeded, value is: " + propValues[1]);
                    mBluetoothService.setRemoteDeviceProperty(address, name, propValues[1]);
                }
        }
    ...
    private void  onAgentAuthorize(String objectPath, String deviceUuid, int nativeData) {
        ...
        if (mA2dp != null &&
            ...
            if (authorized && !BluetoothUuid.isAvrcpTarget(uuid)) {
                ...
                if (SystemProperties.BLUETI_ENHANCEMENT) {
                    ...
                    Log.i(TAG, "" + authorized +
                        "Incoming AVRCP connection from " + address);
                } else{
                    Log.i(TAG, "" + authorized +
            ...
            if (SystemProperties.BLUETI_ENHANCEMENT) {
                Log.i(TAG, "DUN case ");
                if (BluetoothUuid.isDunGateway(uuid)) {
                    Log.i(TAG, "DUN gateway is on");
                    // Check whether BT DUN was enabled by user
                    if (mBluetoothService.isDunEnabled()) {
                        // Check whether DUN is enabled for this device
                        authorized = mBluetoothService.getDunDevicePriority(device) >
                                BluetoothDun.PRIORITY_OFF;
                    }
                    Log.i(TAG, "onAgentAuthorize(): authorized = " + authorized +
                            " for incoming DUN connection from " + address);
                    mBluetoothService.allowIncomingProfileConnect(device, authorized);
                } else {
     ...
     * org.bluez.DunGateway.
     *
     * @param address the MAC address of the disconnected device
     */
    private void onDunDeviceDisconnected(String address) {
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            log ("onDunDeviceDisconnected " + address);
            BluetoothDevice device = mAdapter.getRemoteDevice(address);
            mBluetoothService.handleDunDeviceStateChange(device, BluetoothDun.STATE_DISCONNECTED);
        }
    }
     ...
     */
    private void onDunDeviceConnected(String address) {
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            log ("onDunDeviceConnected " + address);
            BluetoothDevice device = mAdapter.getRemoteDevice(address);
            mBluetoothService.handleDunDeviceStateChange(device, BluetoothDun.STATE_CONNECTED);
        }
    }
 ...
 // BLUETI_ENHANCEMENT
     ...
     * @param propValues Properties (Name-Value) of the Health Device.
     */
    private void onBatteryPropertyChanged(String BattPath, String[] propValues) {
        mBluetoothService.onBatteryPropertyChanged(BattPath, propValues[0], propValues[1]);
    }


    /**
     * Called by native code on rssi level read signal from
     * org.bluez.Device.
     *
     * @param devicePath the object path of the remote device
     * @param status The result code of the RSSI Read response
     * @param rssi The rssi level of the device (BR/EDR or LE)
     */
    private void onRssiLevelResult(String devicePath, short rssi)
    {
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            String address = mBluetoothService.getAddressFromObjectPath(devicePath);
            if (address == null) {
                Log.e(TAG, "onRssiLevelresult: Address of the remote device in null");
                return;
            }

            log("Device : address: " + address + " rssi: " + rssi);

            mBluetoothService.onDeviceRssiLevelReadResult(address, rssi);
        }
    }
