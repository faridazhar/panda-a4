...
class BluetoothDeviceProperties {
        ...
        synchronized(mPropertiesMap) {
            ...
            for (int i = 0; i < properties.length; i++) {
                ...
                if (SystemProperties.BLUETI_ENHANCEMENT) {
                    if (name.equals("UUIDs") || name.equals("Nodes") || name.equals("Services") || name.equals("Batteries")) {
    ...
    if (SystemProperties.BLUETI_ENHANCEMENT) {
        ...
        synchronized(mPropertiesMap) {
            // Only update the property map once, even if we might call BT
            // twice. We can't call BT function with the lock held, since
            // sometimes this code runs from inside BT. This would me lock
            // inversion and a deadlock.
            if (propValues != null /*&& mPropertiesMap.get(address) == null*/) {
                    updatedProps = addProperties(address, propValues);
                }
            }

        // We have added a new remote device or updated its properties.
        // Also update the serviceChannel cache. Do this only if we updated
        // the properties.
        // Again we can't do this under the lock for fear of lock inversion.
