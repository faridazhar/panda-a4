...
final class BluetoothInputProfileHandler {
    ...
    void disconnectInputDevices() {
        // BLUETI_ENHANCEMENT
        if (SystemProperties.BLUETI_ENHANCEMENT) {
            List<BluetoothDevice> devices = getConnectedInputDevices();
            int numberOfConnectedDevices = devices.size();
            debugLog("Number of input devices connected=" + numberOfConnectedDevices);
            //int diconnectMsgLoopCount = 0;
            for (BluetoothDevice device: devices) {
                String objectPath = mBluetoothService.getObjectPathFromAddress(device.getAddress());
                if (objectPath == null ||
                        getInputDeviceConnectionState(device) ==
                            BluetoothInputDevice.STATE_DISCONNECTED) {
                    continue;
                }
                BluetoothDeviceProfileState state = mBluetoothService.getDeviceProfileStateObj(device);
                if (state != null) {
                    debugLog("send message DISCONNECT_HID_OUTGOING");
                    Message msg = new Message();
                    msg.arg1 = BluetoothDeviceProfileState.DISCONNECT_HID_OUTGOING;
                    msg.obj = state;
                    mHidProfileState.sendMessage(msg);
