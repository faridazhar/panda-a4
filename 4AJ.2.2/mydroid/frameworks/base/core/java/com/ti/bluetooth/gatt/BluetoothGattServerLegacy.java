...
/**
 * Back-end class for server profile functionality. The profile class must
 * extend this class and implement all its abstract methods. Note that makes
 * the profile class an android Service in itself, with all related
 * capabilities.
 */
public abstract class BluetoothGattServerLegacy extends Service {
    private static final String TAG = "BluetoothGattServerLegacy";
     ...
     * {@link BluetoothGattServerLegacy#TX_POWER_EXTRA_DEVICE} and the Tx power level
     * is available via {@link BluetoothGattServerLegacy#TX_POWER_EXTRA_LEVEL}.
     */
     ...
     *
     * Used by {@link BluetoothGattServerLegacy#TX_POWER_READ_ACTION}.
     */
    ...
    // the constructor can only be called by other package members
    protected BluetoothGattServerLegacy() { }

    private final IBluetoothGattServerCallback.Stub mBinder = new IBluetoothGattServerCallback.Stub() {
        public void start(int profileId, IBinder srv) {
            mProfileId = profileId;
            mService = IBluetoothGattServer.Stub.asInterface(srv);
        ...
        public void indicateResult(int cookie, int status) {
            synchronized(mIndCookies) {
                GattIndicationStatusCallback cb = mIndCookies.get(cookie);
                if (cb == null)
                    return;
                ...
                mIndCookies.remove(cookie);
            }
        }
    };

    /**
     * Internal function - cannot be overridden. Do not use.
     */
    @Override
    public final IBinder onBind(Intent intent) {
        if (DBG)
            Log.d(TAG, "got onBind with intent: " + intent.getAction());
        if (intent.getAction().equals(ACTION_ATT_PROFILE_SERVICE_START))
            return mBinder;

        return null;
    }

    private class RecordedAttribute {
        byte[] mUuid;
        int mReadReqs;
        int mWriteReqs;
        byte[] mValue;
    ...
    public class GattService {
         ...
         * name is not used.
         */
        public GattService(GattServiceType svcType, GattUuid svcUuid,
                        boolean registerSdp, String name) {
            mServiceIdx = mCurrentServiceIdx++;
            mHandleOffset = 0;
            mUuid = svcUuid;
            mName = name;
        ...
        synchronized public void register() throws RuntimeException {
            ...
            mHandleOffset = 0;
            try {
                mHandleOffset = mService.addService(mProfileId,
            ...
            if (mRegisterSdp) {
                try {
                    mService.registerSdpRecord(mProfileId, mServiceIdx,
        ...
        synchronized public GattChar addChar(GattUuid uuid, int permissions,
            ...
            return new GattChar(valueAttrib, cccAttrib);
        }
        ...
        private void playAttributes() {
            ...
            for (RecordedAttribute r : mAttributes) {
                ...
                if (r.mCb != null) {
                    ...
                    mProfileCallbacks.put(handleValue, r.mCb);
                }

                try {
                    mService.addAttribute(mProfileId, mServiceIdx, handleValue,
    ...
    public class GattChar {
        ...
        GattChar(GattAttribute handle, GattAttribute ccc) {
            mValueAttrib = handle;
            mCccAttrib = ccc;
        }
        ...
        public boolean sendNotification(BluetoothDevice device, byte[] value) {
            ...
            if (DBG)
                Log.d(TAG, "sendNotification to " + address +
                        " length " + value.length);
            try {
                mService.sendNotification(mProfileId, address,
        ...
        synchronized public boolean sendIndication(BluetoothDevice device, byte[] value,
            ...
            int cookie = 0;
            try {
                cookie = mService.sendIndication(mProfileId, device.getAddress(),
            ...
            if (cookie == 0) {
                Log.e(TAG, "failed sending indication profile " + mProfileId +
                        " handle " + mValueAttrib.getHandle());
                return false;
            }

            // no need to protect this - the whole function is synchronized
            mIndCookies.put(cookie, cb);
            return true;
        }
    }
     ...
     */
    protected abstract void registerProfile();
     ...
     * GATT operations.
     */
    protected abstract void unregisterProfile();
}
